...
$geo_latitude =sanitize_text_field($_REQUEST['geo_latitude']);
$geo_longitude = sanitize_text_field($_REQUEST['geo_longitude']);
$map_type =sanitize_text_field($_REQUEST['map_view']);
$website=sanitize_text_field($_REQUEST['website']);
$phone=sanitize_text_field($_REQUEST['phone']);
$listing_logo=sanitize_text_field($_REQUEST['listing_logo']);
$listing_timing=sanitize_text_field($_REQUEST['listing_timing']);
$email=sanitize_email($_REQUEST['email']);
$special_offer=sanitize_text_field($_REQUEST['proprty_feature']);
...
$zooming_factor=sanitize_text_field($_POST['zooming_factor']);
$_REQUEST['imgarr'] = explode(",",sanitize_text_field($_REQUEST['imgarr']));
...
if(function_exists('tmpl_fetch_heading_post_type')){
	...
	$heading_type = tmpl_fetch_heading_post_type($_REQUEST['cur_post_type']);
}
...
if(function_exists('tmpl_single_page_custom_field')){
	$htmlvar_name = tmpl_single_page_custom_field( $_REQUEST['cur_post_type']);
}else{
	global $htmlvar_name;
}
	...
	<div class="listing type-listing listing-type-preview hentry" >  
        ...
        <section class="entry-content">
        	...
        	<div class="share_link">  
               ...
               <script  type="text/javascript" async  src="<?php echo $http; ?>s7.addthis.com/js/250/addthis_widget.js#username=xa-4c873bb26489d97f"></script>
                     <?php if($facebook!=""):?>
            	 ...
            	 <ul class="tabs" data-tab role="tablist">
                     ...
                     <?php if($special_offer!="" && $tmpl_flds_varname['proprty_feature']):?>
                      <li class="tab-title"><a href="#special_offer"><?php echo $tmpl_flds_varname['proprty_feature']['label']; ?></a></li>
                      <?php endif;?>
                      
                      <?php if($video!="" && $tmpl_flds_varname['video']):?>
                      <li class="tab-title"><a href="#listing_video"><?php echo $tmpl_flds_varname['video']['label']; ?></a></li>
                      <?php endif;?>
										...
										<?php else: ?>
											<?php
												...
												foreach($largest_img_arr as $value):	
													...
													$name = end($tmpl);									
													if($val == $name):?>
													<li><img src="<?php echo $value['file'];?>" alt="" height="60" width="60" title="<?php echo $img_title['filename'] ?>" /></li>
											<?php
													endif;
												endforeach;
												endif;
             ...
             <div class="post-meta">  
              ...
              if(function_exists('directory_post_preview_categories_tags') ){
					$_REQUEST['category'] = array_filter($_REQUEST['category']);
