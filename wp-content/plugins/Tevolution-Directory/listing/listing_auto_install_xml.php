...
class WXR_Parser {
	function parse( $file ) {
		...
		$parser = new WXR_Listing_Parser_Regex;
 ...
 */
class WXR_Listing_Parser_Regex {
	var $authors = array();
	var $posts = array();
	...
	function WXR_Listing_Parser_Regex() {
		$this->__construct();
	}
 ...
 */
if ( class_exists( 'WP_Importer' ) ) {
	class Listing_WP_Import extends WP_Importer {
		...
		function Listing_WP_Import() { /* nothing */ }
...
$xml_import = new Listing_WP_Import();

if(file_exists(WP_PLUGIN_DIR.'/Tevolution-Directory/listing-dummy-data.xml') ){
	...
	$directory_search_location = get_option('widget_directory_search_location');
	if(is_array($directory_search_location) && !empty($directory_search_location)){
	...
	update_option('widget_directory_search_location', $directory_search_location);
	$directory_search_location = get_option('widget_directory_search_location');
	$directory_search_location_key3 = '';
	if(is_array($directory_search_location) && !empty($directory_search_location)){ 
	krsort($directory_search_location);
	foreach($directory_search_location as $key1=>$val1)
	{
	...
	$templatic_text = get_option('widget_templatic_text');
	$templatic_text_key4 = '';
	if(is_array($templatic_text) && !empty($templatic_text)){ 
	krsort($templatic_text);
	foreach($templatic_text as $key=>$val)
	{
		$templatic_text_key4 = $key;
	...
	add_action('admin_notices','listing_sample_data_inserted');
	
	/* Add package for all sample listing */
	do_action('tmpl_add_sample_listing_package',CUSTOM_POST_TYPE_LISTING);
}
function listing_sample_data_inserted()
{
	?>
