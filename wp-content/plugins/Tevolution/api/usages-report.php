...
function tmpl_get_site_usages() {
    ...
    $result['site_detail']['current_theme'] = wp_get_theme();
    ...
    $pargs = array('post_type' => 'monetization_package',
        ...
        $paymentinfo = $wpdb->get_results($wpdb->prepare($sql));
        if($paymentinfo)	{
            foreach($paymentinfo as $key => $payment)
            {
                ...
                $transction= $wpdb->get_results($wpdb->prepare($transaction));
