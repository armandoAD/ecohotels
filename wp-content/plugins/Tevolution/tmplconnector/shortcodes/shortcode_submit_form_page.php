...
function tevolution_form_page_template($atts){
	...
	?>
	<!-- shows undefined variable in submit form in console  -->
	...
	$author_id = get_post_field ('post_author', $_REQUEST['pid']);
	if(isset($_REQUEST['pid']) && trim($_REQUEST['pid'])!="" && (((count($post_sql) <= 0) && $author_id!=$current_user->ID) || ($author_id==0 || $current_user->ID==0)))
	{ 
		_e('ERROR: Sorry, you are not allowed to edit this post.','templatic');
	}
	else{
		$category_submit_fields = 0;
		echo '<form name="submit_form" id="submit_form" class="dropzone form_front_style" action="'.$form_action_url.'" method="post" enctype="multipart/form-data">';
			...
			<?php		
				...
				<?php
					...
					if(isset($templatic_settings['templatic-category_custom_fields']) && $templatic_settings['templatic-category_custom_fields']=='Yes')
					{
						$category_submit_fields = 1;
					}
					...
					/*Apply filter hook for create submit from validation info array*/
					...
					$tmpl_cat_class = '';
					if($tmpdata['templatic-category_type'] == 'multi_level_cat')
					{
						$tmpl_cat_class = 'multi_level_cat';
					}
					echo '<div id="submit_category_box" class="'.$tmpl_cat_class.'">';
					...
					{
						echo '<div id="submit_form_terms_and_condition" class="submit_form_terms_and_condition">';
						if($category_submit_fields == 0)
						{
					...
					if($category_submit_fields == 1)
					{
						$display = 'style="display:none;"';
					}
					echo '<input '.$display.' type="button" id="continue_submit_from" name="continue_submit_from" value="'.__('Continue','templatic').'" '.$submit_button.'/>&nbsp;&nbsp;';
					if($category_submit_fields == 0)
					{
		...
		/* for additional post type class */
		$common_post_type = 'singular-'.apply_filters('custom_post_preview',$post_type);
		
		echo '<div id="preview_submit_from_'.$post_type.'"  class="reveal-modal singular-'.$post_type.' '.$common_post_type.' preview_submit_from_data" data-reveal></div>';
