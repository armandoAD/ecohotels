...
function add_to_favorite($post_id,$language='')
{
	...
	echo '<a href="javascript:void(0);" title="'.$remove_fav_text.'" class="removefromfav added" data-id='.$post_id.' onclick="javascript:addToFavourite(\''.$post_id.'\',\'remove\');"><i class="fa fa-heart"></i>'.__('Added','templatic').'</a>';
...
function remove_from_favorite($post_id,$language='')
{
	...
	update_user_meta($current_user->ID, 'user_favourite_post', $user_meta_data); 	
	echo '<a class="addtofav removed" title="'.strip_tags($add_to_favorite).'" href="javascript:void(0);" data-id='.$post_id.'  onclick="javascript:addToFavourite(\''.$post_id.'\',\'add\');"><i class="fa fa-heart-o"></i>';
...
function tevolution_favourite_html($user_id='',$post='')
{
	if(function_exists('tmpl_wp_is_mobile') && !tmpl_wp_is_mobile()){
		...
		if($post->post_type !='post'){
			...
			?>
				<span id="tmplfavorite_<?php echo $post_id;?>" class="fav fav_<?php echo $post_id;?>"><a href="javascript:void(0);" <?php echo $data_reveal_id; ?> id="tmpl_login_frm_<?php echo $post_id; ?>" data-id='<?php echo $post_id; ?>'  class="addtofav <?php echo $class; ?> small_btn"  onclick="javascript:addToFavourite('<?php echo $post_id;?>','add');"><?php echo strip_tags($add_to_favorite);?></a></span>
			<?php } 
		}
	}
}
