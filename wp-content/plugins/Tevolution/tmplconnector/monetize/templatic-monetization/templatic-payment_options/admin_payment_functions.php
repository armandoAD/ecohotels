...
function templ_payment_methods(){ 
	...
	if( @$_GET['status']!='' && @$_GET['id']!='')
	{
		...
		$wpdb->query($wpdb->prepare($updatestatus,wp_kses_post($_GET['id'])));
	}
	?>
	<div class="wrap">
...
function tmpl_currency_settings(){ ?>
	...
	<?php
		...
		{
			update_option('currency_symbol',wp_kses_post($_REQUEST['currency_symbol']));
			update_option('currency_code',wp_kses_post($_REQUEST['currency_code']));
			update_option('currency_pos',wp_kses_post($_REQUEST['currency_pos']));
			update_option('tmpl_price_thousand_sep',wp_kses_post($_REQUEST['tmpl_price_thousand_sep']));
			update_option('tmpl_price_num_decimals',wp_kses_post($_REQUEST['tmpl_price_num_decimals']));
			update_option('tmpl_price_decimal_sep',wp_kses_post($_REQUEST['tmpl_price_decimal_sep']));
		}	
		?>
