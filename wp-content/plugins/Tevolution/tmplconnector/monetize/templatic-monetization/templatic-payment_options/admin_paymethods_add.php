...
if($_POST)
{
	$paymentupdsql = "select option_value from $wpdb->options where option_id=%d";
	$paymentupdinfo = $wpdb->get_results($wpdb->prepare($paymentupdsql,$_GET['id']));
	if($paymentupdinfo)
	{
		foreach($paymentupdinfo as $paymentupdinfoObj)
		{
			...
			$dispaly_title = $_POST['dispaly_title'];
			$payment_logo = $_POST['payment_logo'];
			...
			$option_value['dispaly_title'] = $dispaly_title;
			$option_value['payment_logo'] = $payment_logo;
	...
	$wpdb->query($wpdb->prepare($updatestatus,$_GET['id']));
...
<form action="<?php echo site_url();?>/wp-admin/admin.php?page=monetization&payact=setting&id=<?php echo $_GET['id'];?>&tab=payment_options" method="post" name="payoptsetting_frm" enctype="multipart/form-data">
    ...
    <?php } ?>
	<tr>
		<th><?php echo __('Dispaly payment title?','templatic-admin');?></th>
		<td>
			<?php
			if (!array_key_exists("dispaly_title",$option_value))
			{
				$option_value['dispaly_title'] = 1;
			}
			?>
			<input type="checkbox" name="dispaly_title" id="dispaly_title" value="1" <?php if($option_value['dispaly_title'] == 1){echo "checked='checked'";}?>> 
			<p class="description"><?php echo __('Uncheck this if you don\'t want to dispaly payment gateway title <strong>(If you are unchecking this option, than upload logo other wise this option will not consider)</strong>.','templatic-admin'); ?></p>
		</td>
		</tr>
	<tr>
		<th><?php echo __('Payment Logo','templatic-admin');?></th>
		<td>
			<input type="text" name="payment_logo" id="payment_logo" value="<?php echo $option_value['payment_logo']; ?>" size="50"  /><a data-id="payment_logo" id="Payment Logo" type="submit" class="upload_file_button button"><?php  echo __('Browse','templatic-admin');?></a>
			<p class="description"><?php echo __('Select logo or enter url direct from web (Recommended size 160*25).','templatic-admin'); ?></p>
		</td>
	</tr>
	<tr>
