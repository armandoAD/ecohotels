...
function fetch_meta_options() {
	...
	<?php 
		...
		global $current_user;
		$id = @$_REQUEST['clid'];
		$post_claim_id = $wpdb->get_col($wpdb->prepare("SELECT ID from $wpdb->posts WHERE (post_content = %d OR post_content = %d) AND post_status = 'publish' AND (post_excerpt = 'approved' OR post_excerpt = '') and post_type='claim'",$_REQUEST['post'],$post->ID)); /* FETCH TOTAL NUMBER OF CLAIMS FOR A POST */
				...
				foreach($post_claim_id as $key=>$val) {
					...
					if( $post->ID == $data['post_id'] ) {
						...
						if($current_ip == $user_ip && $user_ip != '') { ?>
							<a class="claimed" href="javascript:void(0)"><?php _e(apply_filters('tmpl_already_claimed_text','Already Claimed'),'templatic'); ?></a>
						<?php 
						} else{ ?>					
...
function tevolution_claimowner_ship() {
	...
	if(@$_REQUEST['claimer_name']) {
		...
		/* POST CLAIM FORM VALUES */
		$yourname = sanitize_text_field($_REQUEST['claimer_name']);
		$youremail = sanitize_email($_REQUEST['claimer_email']);
		$your_number = sanitize_text_field($_REQUEST['claimer_contact']);
		$c_number = sanitize_text_field($_REQUEST['claimer_contact']);
...
function tmpl_after_title_returns() {
	global $post;
	if(get_post_meta($post->ID,'is_verified',true) == 1) {
		echo '<span  data-tooltip aria-haspopup="true" data-options="disable_for_touch:true" class="fa-stack has-tip tip-right" title="'.__("Verified","templatic").'"><i class="fa fa-certificate fa-stack-2x"></i><i class="fa fa-check fa-stack-1x"></i></span>';
	} 
}
?>