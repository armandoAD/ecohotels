...
function tmpl_custom_setup_tabs_fn() {
               ...
               if ($args):
                         foreach ($args as $key => $_args) {
                                   if (post_type_exists($_args["post_slug"])) {
                                             ...
                                             /* Single post Type sider bar  */
                                             register_sidebars(1, array('id' => '' . $_args["post_slug"] . '_above_mobile_listing', 'name' => sprintf(__('%s Category Mobile Header', 'templatic'), ucfirst($_name)), 'description' => sprintf(__('Display widgets %s above pages.', 'templatic'), $_name), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3 class="widget-title">', 'after_title' => '</h3>'));

                                             /* Add post submit side bar */
                                             register_sidebars(1, array('id' => 'add_' . $_args["post_slug"] . '_submit_sidebar', 'name' => sprintf(__('Submit %s - Sidebar', 'templatic'), ucfirst($_name)), 'description' => sprintf(__('Display widgets in a sidebar that appears on %s submission pages.', 'templatic'), $_name), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3 class="widget-title">', 'after_title' => '</h3>'));
     ...
     function templatic_edit_taxonomy_columns($columns) {
               ...
               if ($fld_meta_info->have_posts()) {
                         while ($fld_meta_info->have_posts()) : $fld_meta_info->the_post();
                                   ...
                                   if ($post->post_title != '') {
                                             $columns1[$post->post_name] = apply_filters('tmpl_column_title',$post->post_title);
                                   }
                         endwhile;
                         wp_reset_query();
               }
     ...
     function templatic_manage_taxonomy_columns($column, $post_id) {
               ...
               switch ($column) {
                         case 'post_image' :
									/* get logo url */
									$logo_url = get_post_meta($post->ID,'listing_logo',true);
                                   ...
                                   if (has_post_thumbnail()):
									...
									elseif($logo_url !=''):
										  echo '<a href="' . site_url() . '/wp-admin/post.php?post=' . $post_id . '&action=edit">';
									?>
										 <img src="<?php echo $logo_url; ?>" width="80px" width="60" height="60"/>
									<?php
										echo '</a>';
                                   else:
                         ...
                         case 'price_package' :
                                   ...
                                   $trans_object = $wpdb->get_row("select * from $transactions as t where 1=1  AND (package_type is NULL OR package_type=0) AND post_id = {$post_id} order by t.trans_id desc LIMIT 0,1 ");
					               $package_id = get_post_meta($post_id, 'package_select', true);
				if ($package_id != "") {
                                             ...
                                             if (function_exists('fetch_currency_with_position')) {
												 ...
												 {
													$paid_amount = display_amount_with_currency_plugin($trans_object->payable_amt);
												 }
                                             }
                                             $transaction_db_table_name = $wpdb->prefix . 'transactions';
                                             $trans_date = $wpdb->get_var("select payment_date from $transaction_db_table_name t where post_id = '" . $post_id . "' AND (package_type is NULL OR package_type=0) order by t.trans_id DESC"); /* change it to calculate expired day as per transactions */
                                             if (!isset($trans_date))
					     ...
					     $package_amount = fetch_currency_with_position(get_post_meta($package_id,'package_amount',true));
					     echo '<p>' . __('Total Price: ', 'templatic-admin') . $package_amount . '</p>';
                         ...
                         case $column :
                                   if (get_post_meta($post_id, $column, true)) {
                                             ...
                                             if (is_array($value)) {
                                                       ...
                                                       echo apply_filters('tmpl_column_value',$value,$column);
                                             }
                                   } else {
                                             echo "";
                                   }
     ...
     function add_category_price_field($tax, $screen) {
               ...
               <tr class="form-field">
                    ...
                    <td><input type="text"  name="cat_price" id="cat_price" value="<?php
                         ...
                         ?>"  size="20"/>
                         <p class="description"><?php echo sprintf(__('To change currency <a href="%sadmin.php?page=monetization&tab=currency_settings" target= "_blank" >click here</a>', 'templatic'), admin_url()); ?>.</p>
                    </td>
               </tr>
               <?php
     }
               ...
               switch ($column) {
                         case 'email':
                                   ...
                                   <tr class="payment-success-admin alternate">
                                        <td><label class="form-textfield-label"><?php echo __('Successful payment notification for membership package to admin notification', 'templatic'); ?></label></td>
								   ...
								   <tr class="pre-payment-success-user">
                                        <td><label class="form-textfield-label"><?php echo __('PreBank transfer notification to user', 'templatic-admin'); ?></label></td>

                                        <td>
                                             <a href="javascript:void(0);" onclick="open_quick_edit('pre-payment-success-user', 'edit-pre-payment-success-user')"><?php echo __("Quick Edit", 'templatic-admin'); ?></a>
                                             | 
                                             <a href="javascript:void(0);" onclick="reset_to_default('pre_payment_success_email_subject_to_user', 'pre_payment_success_email_content_to_user', 'pre-payment-success-user');"><?php echo __("Reset", 'templatic'); ?></a>
                                             <span class="spinner" style="margin:0 18px 0;"></span>
                                             <span class="qucik_reset"><?php echo __("Data reset", 'templatic-admin'); ?></span>
                                        </td>
                                   </tr>
                                   <tr class="edit-pre-payment-success-user" style="display:none">
                                        <td width="100%" colspan="2">
                                             <h4 class="edit-sub-title">Quick Edit</h4>
                                             <table width="98%" align="left" class="tab-sub-table">
                                                  <tr>
                                                       ...
                                                       <td width="90%" style="line-height:10px">
                                                            <input type="text" name="pre_payment_success_email_subject_to_user" id="pre_payment_success_email_subject_to_user" value="<?php
                                                            if (isset($tmpdata['pre_payment_success_email_subject_to_user'])) {
                                                                      echo $tmpdata['pre_payment_success_email_subject_to_user'];
                                                            } else {
                                                                      _e('Submission pending payment', 'templatic');
                                                            }
                                                            ?>"/>
                                                  ...
                                                  <tr>
                                                       ...
                                                       <td width="90%" style="line-height:10px">
                                                            ...
                                                            $settings = array(
                                                                      'wpautop' => false, /* use wpautop? */
                                                                      'media_buttons' => false, /* show insert/upload button(s) */
                                                                      'textarea_name' => 'pre_payment_success_email_content_to_user', /* set the textarea name to something different, square brackets [] can be used here */
                                                                      'textarea_rows' => '7', /* rows="..." */
                                                                      'tabindex' => '',
                                                                      'editor_css' => '<style>.wp-editor-wrap{width:640px;margin-left:0px;}</style>', /* intended for extra styles for both visual and HTML editors buttons, needs to include the <style> tags, can use "scoped". */
                                                                      'editor_class' => '', /* add extra class(es) to the editor textarea */
                                                            ...
                                                            if ($tmpdata['pre_payment_success_email_content_to_user'] != "") {
                                                                      $content = stripslashes($tmpdata['pre_payment_success_email_content_to_user']);
                                                            } else {
                                                                      $content = __("<p>Dear [#to_name#],</p><p>A payment from username [#user_login#] is now pending on a submission or subscription to one of your plans.</p><p>[#transaction_details#]</p><p>Thanks!<br/>[#site_name#]</p>", 'templatic-admin');
                                                            }
                                                            wp_editor($content, 'pre_payment_success_email_content_to_user', $settings);
                                                            ?>
                                                       </td>
                                                  </tr>
                                                  <tr>
                                                       <td colspan="2">
                                                            <div class="buttons">
                                                                 <div class="inline_update">
                                                                      <a class="button-primary save  alignleft quick_save" href="javascript:void(0);" accesskey="s"><?php echo __("Save Changes", 'templatic-admin'); ?></a>
                                                                      <a class="button-secondary cancel alignright" href="javascript:void(0);" onclick="open_quick_edit('edit-pre-payment-success-user', 'pre-payment-success-user')" accesskey="c"><?php echo __("Cancel", 'templatic-admin'); ?></a>
                                                                      <span class="save_error" style="display:none"></span><span class="spinner"></span>
                                                                 </div>
                                                            </div>	
                                                       </td>
                                   ...
                                   <tr class="edit-update-listing-notification" style="display:none">
								    ...
								    <tr class="update-listing-notification-admin">
                                        <td><label class="form-textfield-label"><?php echo __('Listing updated notification to admin', 'templatic-admin'); ?></label></td>

                                        <td>
                                             <a href="javascript:void(0);" onclick="open_quick_edit('update-listing-notification-admin', 'edit-update-listing-notification-admin')"><?php echo __("Quick Edit", 'templatic-admin'); ?></a>
                                             | 
                                             <a href="javascript:void(0);" onclick="reset_to_default('update_listing_notification_admin_subject', 'update_listing_notification_admin_content', 'update-listing-notification-admin');"><?php echo __("Reset", 'templatic'); ?></a>
                                             <span class="spinner" style="margin:0 18px 0;"></span>
                                             <span class="qucik_reset"><?php echo __("Data reset", 'templatic-admin'); ?></span>
                                        </td>
                                   </tr>
                                   <tr class="edit-update-listing-notification-admin" style="display:none">
                                        <td width="100%" colspan="2">
                                             <h4 class="edit-sub-title">Quick Edit</h4>
                                             <table width="98%" align="left" class="tab-sub-table">
                                                  <tr>
                                                       ...
                                                       <td width="90%" style="line-height:10px">
                                                            <input type="text" name="update_listing_notification_admin_subject" id="update_listing_notification_admin_subject" value="<?php
                                                            if (isset($tmpdata['update_listing_notification_admin_subject'])) {
                                                                      echo $tmpdata['update_listing_notification_admin_subject'];
                                                            } else {
                                                                      _e('[#post_type#] ID #[#submition_Id#] has been updated', 'templatic');
                                                            }
                                                            ?>"/>
                                                  ...
                                                  <tr>
                                                       ...
                                                       <td width="90%" style="line-height:10px">
                                                            ...
                                                            $settings = array(
                                                                      'wpautop' => false, /* use wpautop? */
                                                                      'media_buttons' => false, /* show insert/upload button(s) */
                                                                      'textarea_name' => 'update_listing_notification_admin_content', /* set the textarea name to something different, square brackets [] can be used here */
                                                                      'textarea_rows' => '7', /* rows="..." */
                                                                      'tabindex' => '',
                                                                      'editor_css' => '<style>.wp-editor-wrap{width:640px;margin-left:0px;}</style>', /* intended for extra styles for both visual and HTML editors buttons, needs to include the <style> tags, can use "scoped". */
                                                                      'editor_class' => '', /* add extra class(es) to the editor textarea */
                                                            ...
                                                            if ($tmpdata['update_listing_notification_admin_content'] != "") {
                                                                      $content = stripslashes($tmpdata['update_listing_notification_admin_content']);
                                                            } else {
                                                                      $content = __("<p>Dear [#to_name#],</p><p>[#post_type#] ID #[#submition_Id#] has been updated on your site.</p><p>You can review it again by clicking on its title in this email or through your admin dashboard.</p>[#information_details#]<br><p>[#site_name#]</p>", 'templatic-admin');
                                                            }
                                                            wp_editor($content, 'update_listing_notification_admin_content', $settings);
                                                            ?>
                                                       </td>
                                                  </tr>                                                    
                                                  <tr>
                                                       <td colspan="2">
                                                            <div class="buttons">
                                                                 <div class="inline_update">
                                                                      <a class="button-primary save  alignleft quick_save" href="javascript:void(0);" accesskey="s"><?php echo __("Save Changes", 'templatic-admin'); ?></a>
                                                                      <a class="button-secondary cancel alignright" href="javascript:void(0);" onclick="open_quick_edit('edit-update-listing-notification-admin', 'update-listing-notification-admin')" accesskey="c"><?php echo __("Cancel", 'templatic-admin'); ?></a>
                                                                      <span class="save_error" style="display:none"></span><span class="spinner"></span>
                                                                 </div>
                                                            </div>	
                                                       </td>
