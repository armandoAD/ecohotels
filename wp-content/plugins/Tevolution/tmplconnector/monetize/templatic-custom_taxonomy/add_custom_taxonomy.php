	...
	if($post_slug)
	 {
		$cutom_post[$post_slug] =
					array(	'label' 			=> stripslashes(sanitize_text_field($_POST['post_name'])),
							'labels' 			=> array(	'name' 			 => 	stripslashes(sanitize_text_field($_POST['post_name'])),
													'singular_name' 	 => 	stripslashes(sanitize_text_field($_POST['post_name'])),
													'menu_name' 	 => 	stripslashes(sanitize_text_field($_POST['post_name'])),
		...
		update_post_meta($post_heading, 'post_type_'.$post_slug.'' , $post_slug);
	...
	if($_SESSION['taxonomy_post_type'])
	  {
		...
		global $wpdb;
		$wpdb->query("Update $wpdb->posts SET post_type = '".str_replace(" ","",wp_kses_post(strtolower($_POST['post_slug'])))."' where post_type = '".str_replace(" ","",strtolower($original[$_SESSION['taxonomy_post_type']]['label']))."'");
		$wpdb->query("Update $wpdb->term_taxonomy SET taxonomy = '".wp_kses_post($_POST['taxonomy_slug'])."' where taxonomy = '".$tax_slug."'");
		$wpdb->query("Update $wpdb->term_taxonomy SET taxonomy = '".wp_kses_post($_POST['tag_slug'])."' where taxonomy = '".$tag_slug."'");
