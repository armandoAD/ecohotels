...
function filter_my_theme_nav_bars($items, $args) {
	...
	if(in_array($args->theme_location,$theme_locations))
	{
		if($current_user->ID){
			...
			$loginlink = '<li class="tmpl-login' . ((isset($_REQUEST['ptype'])) && ($_REQUEST['ptype'] =='login')? ' current_page_item' : '') . '" ><a data-reveal-id="tmpl_reg_login_container" href="javascript:void(0);" onClick="tmpl_login_frm();">' . __('Login','templatic') . '</a></li>'; 
		}
		if($current_user->ID){
			...
			if($users_can_register){				
				$reglink = '<li class="tmpl-login' . ((isset($_REQUEST['ptype'])) && ($_REQUEST['ptype'] =='register')? ' current_page_item' : '') . '"><a data-reveal-id="tmpl_reg_login_container" href="javascript:void(0);" onClick="tmpl_registretion_frm();">' . __('Register','templatic') . '</a></li>';
							...
							jQuery(document).ready(function(){
								var settings = {
									...
									onSuccess:function(files,data,xhr)
									{
										...
										//jQuery('#<?php echo $key; ?>').val(img_name);
		...
		{			
			if($val['type']=='texteditor')
			{
				...
				if(is_admin())
					...
					echo $val['outer_st'].$label.$str.$val['outer_end'];
			}
        }
		}
	}
...
function get_tevolution_login_permalink(){
	...
	$login_page = $wpdb->get_row("SELECT ID FROM $wpdb->posts WHERE $wpdb->posts.post_name = 'login' and $wpdb->posts.post_type = 'page'");
...
function get_tevolution_register_permalink(){
	...
	$register_page = $wpdb->get_row("SELECT post_title,ID FROM $wpdb->posts WHERE $wpdb->posts.post_content LIKE '%[tevolution_register%' and $wpdb->posts.post_type = 'page' ORDER BY ID DESC LIMIT 0,1");
...
function get_tevolution_profile_permalink(){
	...
	$profile_page = $wpdb->get_row("SELECT post_title,ID FROM $wpdb->posts WHERE $wpdb->posts.post_content LIKE '%[tevolution_profile%' and $wpdb->posts.post_type = 'page' ORDER BY ID DESC LIMIT 0,1");
...
function tmpl_ajax_check_user_email()
{
	...
	{
		$user_fname = sanitize_text_field($_REQUEST['user_fname']);
