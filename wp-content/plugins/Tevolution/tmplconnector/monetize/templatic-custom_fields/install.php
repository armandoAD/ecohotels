...
/* Files related to monetization */
	...
	add_filter('body_class','remove_admin_bar_onpreview',10,2);/* call body class for remove admin bar */
...
$extension_file=array('.jpg','.JPG','jpeg','JPEG','.png','.PNG','.gif','.GIF','.jpe','.JPE','.zip','.ZIP');
 ...
 */
function remove_admin_bar_onpreview($classes,$class){
		...
		*/
		...
		$heading_post_type_arr = substr($heading_post_type_arr,0,-1);
		 ...
		 if(count($taxonomy_name) == 0)
		 {
			...
			if(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_custom_filed') == ''){
				global $sitepress;
				$current_lang_code= ICL_LANGUAGE_CODE;
				$default_language = $sitepress->get_default_language();	
				/* Insert wpml  icl_translations table*/
				$sitepress->set_element_language_details($taxonomy_name->ID, $el_type='post_custom_fields', $taxonomy_name->ID, $current_lang_code, $default_language );
				if(function_exists('wpml_insert_templ_post'))
					wpml_insert_templ_post($taxonomy_name->ID,'custom_fields'); /* insert post in language */
			}
		...
		if(count($post_category) == 0)
		 {
			...
			if(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_custom_filed') == ''){
				...
				$default_language = $sitepress->get_default_language();	
				/* Insert wpml  icl_translations table*/
				$sitepress->set_element_language_details($post_category->ID, $el_type='post_custom_fields', $post_category->ID, $current_lang_code, $default_language );
				if(function_exists('wpml_insert_templ_post'))
					wpml_insert_templ_post($post_category->ID,'custom_fields'); /* insert post in language */
			}
		 }
		...
		if(count($post_title) == 0)
		 {
			...
			if(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_custom_filed') == ''){
				...
				$default_language = $sitepress->get_default_language();	
				/* Insert wpml  icl_translations table*/
				$sitepress->set_element_language_details($post_title->ID, $el_type='post_custom_fields', $post_title->ID, $current_lang_code, $default_language );
				if(function_exists('wpml_insert_templ_post'))
					wpml_insert_templ_post($post_title->ID,'custom_fields'); /* insert post in language */
			}
		...
		if(count($field_label) == 0)
		{
		 ...
		 if(count($post_images) == 0)
		 {
			...
			if(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_custom_filed') == ''){
				...
				$default_language = $sitepress->get_default_language();	
				/* Insert wpml  icl_translations table*/
				$sitepress->set_element_language_details($post_images->ID, $el_type='post_custom_fields', $post_images->ID, $current_lang_code, $default_language );
				if(function_exists('wpml_insert_templ_post'))
					wpml_insert_templ_post($post_images->ID,'custom_fields'); /* insert post in language */
			}
		 }
        ...
        if(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_custom_filed') == ''){
			update_option('tmpl_wpml_custom_filed','inserted');
		}		
...
function post_page_setting_data($column)
{
	...
	?>
		...
		<ul id="tev_general_settings" class="filter-links">
			...
			?>
                        <?php 
                        /* add theme support for not show in theme */
                        /* This condition for Publisher/digiseller */
                        if(!current_theme_supports('theme_not_support_claim_ownership')): ?>
		...
		<table id="submit_page_settings" class="tmpl-general-settings form-table active-tab">
			...
			<tr id="category_specific_search_fields" >
                <th>
                    <label><?php echo __('Category specific advanced search','templatic-admin');	$category_specific_search_fields =  @$tmpdata['templatic-category_custom_search_fields']; if(!isset($category_specific_search_fields) && $category_specific_search_fields == ''){update_option('templatic-category_custom_search_fields','No');}?></label>
                </th>
                <td> 
				
				
					<div class="input-switch">
						<input type="checkbox"  id="templatic-category_custom_search_fields" name="templatic-category_custom_search_fields" value="Yes" <?php if($category_specific_search_fields == 'Yes' ){?>checked="checked"<?php }?> />
						<label for="templatic-category_custom_search_fields" class="checkbox">&nbsp;<?php echo __('Enable','templatic-admin');?></label>
					</div>
             
                    <p class="description"><?php echo __('Displays different fields for different categories on the Advanced Search widget','templatic-admin');?></p>
                </td>
            </tr>
			 <tr>
					...
					<div class="element">
						 <div class="input_wrap">
						   ...
						   <select id="templatic-category_type" name="templatic-category_type" style="vertical-align:top;width:200px;" >							
							...
							<option value="multi_level_cat" <?php if($templatic_category_type == 'multi_level_cat' ) { echo "selected=selected";  } ?>><?php echo __('Multi Level Category Selection','templatic-admin'); ?></option>
						   </select> 
					</div>
					</div>
				   <label for="ilc_tag_class"><p class="description"><?php echo __('Specify the format in which you want to display the categories on ','templatic-admin');?><a href="https://templatic.com/docs/tevolution-guide/#categorydisplay"><?php echo __('Submit page.','templatic-admin'); ?></a></p></label>
				</td>
			 </tr>
		...
		if(!current_theme_supports('remove_tevolution_sharing_opts')){ ?>                         
			...
			<tr>
				<th><label><?php echo __('Facebook Access Token ','templatic-admin'); $fb_access_token =  @$tmpdata['fb_access_token'];?></label></th>
				<td>
					<label for="fb_access_token">
						<input id="fb_access_token" type="text" value="<?php if(isset($fb_access_token)){ echo @$fb_access_token;}  ?>" name="fb_access_token" placeholder="App ID|App Secret">
					</label>
					<p class="description"><?php echo __('Enter facebook access token for share count. Enter App Id and App Secret together as access token.','templatic-admin');?></p>
				</td>
			</tr>
			<?php 
		} 
...
function tevolution_custom_fields_notice(){
	...
	if((isset($_REQUEST['ctab']) && ($_REQUEST['ctab']=='custom_fields' || $_REQUEST['page']=='custom_taxonomies_permalink' ) && !isset($_REQUEST['activated'])) || (isset($_REQUEST['taxonomy']) && array_key_exists($_REQUEST['taxonomy'],$taxonomy)) || (isset($_REQUEST['taxonomy']) && array_key_exists($_REQUEST['taxonomy'],$tag)) || (isset($_REQUEST['page']) && ($_REQUEST['page']=='custom_setup' || $_REQUEST['page']=='templatic_settings' ) )){
			...
			if(isset($_POST['tevolution_query_cache']) && $_POST['tevolution_query_cache']!=''){				
				update_option('tevolution_cache_disable',wp_kses_post($_REQUEST['tevolution_cache_disable']));
			}
...
function successfull_return_paypal_content_message(){
	...
	$content = sprintf(__('You have now upgraded to a new package %s You paid %s to upgrade your package.','templatic'),"<strong>".$pkgname."</strong>.<br/>","<strong>".display_amount_with_currency_plugin($paid_amount)."</strong>");
	echo "<h2>".$subject."</h2>";
	echo "<p>".$content."</p>";
	echo "<p>".$subcontent."</p>";
}
