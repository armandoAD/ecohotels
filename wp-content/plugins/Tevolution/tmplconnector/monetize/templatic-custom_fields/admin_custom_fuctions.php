...
if(isset($_POST['Verify']) && $_POST['Verify'] !='') {
	...
	$response = wp_remote_get('https://templatic.com/members/login_api.php',$arg );
	...
	if(!is_wp_error( $response ) ) {
		...
		if(isset($_POST['licencekey']) && $_POST['licencekey'] !='') { 
			if(strstr($response['body'],'error_message')) {
				...
				update_option('templatic_licence_key_',wp_kses_post($_POST['licencekey']));
		...
		/* license key verification start */
		...
		if(!function_exists('curl_version')) {
			$msg .= 'cURL is not installed on this server.';
		}
		
		if(fsockopen($_SERVER['HTTP_HOST'],80)) {
			//$msg .= 'port 80 is not blocked by server';
		} else {
			$msg .= 'Port 80 is blocked by server.';
		}
		
		if($msg)
			$msg .= ' Please contact host provider.';
		
		/* check if port 80 is block or not */
		
		update_option('templatic_licence_key','{"error_message":"WP HTTP Error: '.$msg.'"}');
...
function tevolution_error_message_host() {
	...
	echo "<span style='color:red'><br>".__("WP HTTP Error: couldn't connect to host.",'templatic-admin')."</span><br><br>";
	...
	if(!function_exists('curl_version')) {
		$msg .=  '<h3 class="hndle">'.__('Please ask your web host to enable cURL.','templatic-admin').'</h3>';
		$msg .=  '<p style="color:red"><b>Note: </b>'.__('License verification failed. In order to activate the license, you should have <b> cURL enabled </b> on your site. Please ask your website hosting company to enable cURL and try again.','templatic-admin').'</p>'; 
	}
	
	/* check if port is block or not */
	if(fsockopen($_SERVER['HTTP_HOST'],80)) {
		/* do nothing */
	} else {
		$msg .=  '<h3 class="hndle">'.__('Please ask your web host to enable port 80.','templatic-admin').'</h3>';
		$msg .=  '<p style="color:red"><b>Note: </b>'.__('License verification failed. </b> Port 80 is blocked on your web hosting</b> . Please ask your website hosting company to enable port 80 and try again.','templatic-admin').'</p>'; 
	}
...
function tevolution_error_message_error($message) {
	...
	echo "<p>".__('Please ','templatic-admin'). "<a href='http://templatic.com/contact/'>" .__('contact us','templatic-admin') . "</a> ". __('if you require any further assistance with the activation.','templatic-admin'). "</p>";
}
					...
					<h2><?php echo __('Verify your product license','templatic-admin'); ?></h2>
                         ...
                         <div class="inside">
                                ...
                                <div id="licence_fields">
                                   <input type="password" name="licencekey" id="licencekey" value="<?php echo get_option('templatic_licence_key_'); ?>" size="30" max-length="36" PLACEHOLDER="templatic.com license code"/>
...
function tmpl_overview_box() {
	...
	if($post_counts<=0 ){
		$dummy_theme_message .= '<div class="tmpl-auto-install-yb"><h4>Your site not looking like our online demo? </h4><span><a class="button button-primary" href="'. admin_url( 'themes.php' ).'">Click here</a>to install sample data. </span></div>';
	}
...
function tmpl_taxonomy_metabox_insert($post_id) {
   ...
   /* Finish image gallery sorting */
	...
	{
		update_post_meta(@$_POST['post_ID'], 'template_post_type', wp_kses_post(@$_POST['template_post_type']));
	}
	if(is_templ_wp_admin() && isset($_POST['map_image_size']))			
		update_post_meta(intval($_POST['post_ID']), 'map_image_size', wp_kses_post($_POST['map_image_size']));
	if(is_templ_wp_admin() && isset($_POST['map_width']))			
		update_post_meta(intval($_POST['post_ID']), 'map_width', wp_kses_post(($_POST['map_width'])));
	if(is_templ_wp_admin() && isset($_POST['map_height']))			
		update_post_meta(intval($_POST['post_ID']), 'map_height', wp_kses_post($_POST['map_height']));
	if(is_templ_wp_admin() && isset($_POST['map_center_latitude']))			
		update_post_meta(intval($_POST['post_ID']), 'map_center_latitude', wp_kses_post($_POST['map_center_latitude']));
	if(is_templ_wp_admin() && isset($_POST['map_center_longitude']))
		update_post_meta(intval($_POST['post_ID']), 'map_center_longitude', wp_kses_post($_POST['map_center_longitude']));
	if(is_templ_wp_admin() && isset($_POST['map_type']))
		update_post_meta(intval($_POST['post_ID']), 'map_type', wp_kses_post($_POST['map_type']));
	if(is_templ_wp_admin() && isset($_POST['map_display']))
		update_post_meta(intval($_POST['post_ID']), 'map_display', wp_kses_post($_POST['map_display']));
	if(is_templ_wp_admin() && isset($_POST['map_zoom_level']))
		update_post_meta(intval($_POST['post_ID']), 'map_zoom_level', wp_kses_post($_POST['map_zoom_level']));
	if(is_templ_wp_admin() && isset($_POST['zooming_factor']))
		update_post_meta(intval($_POST['post_ID']), 'zooming_factor', wp_kses_post($_POST['zooming_factor']));
	if(is_plugin_active('thoughtful-comments/fv-thoughtful-comments.php')){
		if(is_templ_wp_admin() && isset($_POST['author_moderate']))
			update_post_meta(intval($_POST['post_ID']), 'author_moderate', wp_kses_post($_POST['author_moderate']));
	}	
	/* verify nonce */
    ...
    $pt_metaboxes = get_post_admin_custom_fields_templ_plugin(sanitize_text_field($_POST['post_type']));    
    ...
    foreach ($pt_metaboxes as $pt_metabox) { /* On Save.. this gets looped in the header response and saves the values submitted */
		...
		if($pt_metabox['type'] == 'text' OR $pt_metabox['type'] == 'oembed_video' OR $pt_metabox['type'] == 'select' OR $pt_metabox['type'] == 'checkbox' OR $pt_metabox['type'] == 'textarea' OR $pt_metabox['type'] == 'radio'  OR $pt_metabox['type'] == 'upload' OR $pt_metabox['type'] == 'date' OR $pt_metabox['type'] == 'multicheckbox' OR $pt_metabox['type'] == 'geo_map' OR $pt_metabox['type'] == 'texteditor' OR $pt_metabox['type'] == 'range_type')
			...
			if($pt_metabox['type'] == 'geo_map'){ 
				update_post_meta($pID, 'address', sanitize_text_field($_POST['address']));
				update_post_meta($pID, 'geo_latitude', wp_kses_post($_POST['geo_latitude']));
				update_post_meta($pID, 'geo_longitude', wp_kses_post($_POST['geo_longitude']));
			}
			...
			{
				update_post_meta($pID, $pt_metabox["name"], wp_kses_post($_POST[$var]));
			}
			elseif($_POST[$var] == "")
			{
	...
	if($_POST['alive_days'] != '' || $_POST['package_select'] != ''){
		...
		{
			$featured_type = get_post_meta($pID, 'featured_type',true);
			$total_amount = $monetization->tmpl_get_payable_amount($_POST['package_select'],$featured_type,$_POST['category']);
		}
		...
		if($pID!=''){
			...
			$post_trans_id  = $wpdb->get_row($wpdb->prepare("select * from $transaction_db_table_name where post_id = %d AND (package_type is NULL OR package_type=0)",$pID)) ;
...
function tevolution_custom_meta_box_content($post, $metabox ) {
    ...
    if($pt_metaboxes) {
		...
		if($pt_metabox['type'] == 'text' OR $pt_metabox['type'] == 'select' OR $pt_metabox['type'] == 'radio' OR $pt_metabox['type'] == 'checkbox' OR $pt_metabox['type'] == 'textarea' OR $pt_metabox['type'] == 'upload' OR $pt_metabox['type'] == 'date' OR $pt_metabox['type'] == 'multicheckbox' OR $pt_metabox['type'] == 'texteditor' OR $pt_metabox['type'] == 'range_type'  && $pt_metabox["name"] !='post_content')
				...
				if($style_class=="")
				{
					$style_class=$pt_metabox['htmlvar_name'];
				}
				else
				{
					$style_class=$pt_metabox['htmlvar_name']." row-".$pt_metabox['style_class'];
				}
				if (@$pt_metaboxvalue == ""  ) {
					...
					echo  "</td></tr>";
						...
						if($array){
							...
							foreach ( $array as $id => $option ) {
								...
								echo  "\t\t".'<div class="multicheckbox"><input type="checkbox" '.$checked.' id="'.$pt_id.'_'.$chkcounter.'" class="pt_input_radio" value="'.$option.'" name="'. $pt_metabox["name"] .'[]" />  <label for="'.$pt_id.'_'.$chkcounter.'">' . $option_title[($i-1)] .'</label></div>'."\n";				$i++;
							}
							echo "</div>";
						}
						echo  '<p class="description">'.$pt_metabox['desc'].'</p>'."\n";
					...
					/* html for image upload for submit form backend end */
					 $pt_metaboxvalue = get_post_meta($post->ID,$pt_metabox["name"],true);
						...
						<script type="text/javascript" async>
							...
							//var image_thumb_src = '<?php echo  get_template_directory_uri().'/images/tmp';?>/';
							jQuery(document).ready(function(){
								var settings = {
									...
									onSuccess:function(files,data,xhr)
									{
										...
										if(id_name[1] == 'jpeg' || id_name[1] == 'jpeg' || id_name[1] == 'jpeg' || id_name[1] == 'jpeg'){
											var attachment_url = image_thumb_src+id_name[0]+"."+id_name[1];
										}else{	
											var img_name = "<?php echo TEVOLUTION_PAGE_TEMPLATES_URL;?>/images/attachment.jpg";
											var attachment_url = image_thumb_src+id_name[0]+"."+id_name[1];
										}	
										...
										//console.log(jQuery('#<?php echo $pt_metabox["name"]; ?>'));
										jQuery('#<?php echo $pt_metabox["name"]; ?>').val(attachment_url);
...
function tevolution_featured_list_fn($post_id){
	...
	echo __('Selecting a package is mandatory.','templatic');
	echo '</td>';
	echo '</tr>';
	echo "</tbody>";
	echo "</table>";
		...
		for($i=0;$i<count($_REQUEST['cf']);$i++)
		{
			...
			if(isset($_REQUEST['action']) && $_REQUEST['action'] !='' && $_REQUEST['action'] !='delete'){
				...
				if((isset($_REQUEST['action']) && $_REQUEST['action'] == 'cancel' ) || (isset($_REQUEST['action2']) && $_REQUEST['action2'] == 'cancel' ))
				{
					...
					$content = apply_filters('pending_listing_notification_content',$tmpdata['payment_cancelled_content'],$orderinfo);
					if(!$content)
					{
...
/* remove some admin link for subscriber user */
add_action('admin_menu', 'tmpl_remove_subscriber_menus');
function tmpl_remove_subscriber_menus()
{
    global $current_user;
	if ( !is_multisite() ) {
		if ( ! user_can( $current_user, "subscriber" ) ){ // Check user object has not got subscriber role
			...
			foreach($post_typearr as $posttype){
				remove_menu_page( 'edit.php?post_type='.$posttype );
			}
...
if(strstr($_SERVER['REQUEST_URI'],'plugins.php') || strstr($_SERVER['REQUEST_URI'],'update.php') || strstr($_SERVER['REQUEST_URI'],'update-core.php') ){
	...
	$dateTimestamp1=get_option('tmpl_tev_update_check_date');
	if( trim($dateTimestamp1!="") ) {
		$dateTimestamp1=strtotime($dateTimestamp1);
	} else{
		update_option('tmpl_tev_update_check_date',date('Y-m-d H:i:s'));
	}
	$dateTimestamp1=strtotime(get_option('tmpl_tev_update_check_date'));
	...
	if ($hour_diff > 3 || $is_update_page==1) {
		require_once(TEVOLUTION_PAGE_TEMPLATES_DIR.'wp-updates-plugin.php');
		new WPUpdatesTevolutionUpdater( 'http://templatic.com/_data/updates/api/index.php', plugin_basename(__FILE__) );
		if($is_update_page==0) {
			update_option('tmpl_tev_update_check_date',date('Y-m-d H:i:s'));
		}
	}
...
function tevolution_post_upgrade_insert(){
	...
	if(($pagenow=='plugins.php' || $pagenow=='themes.php') && get_option('tev_post_upgrade_page') !='inserted'){
		 ...
		 elseif(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_post_upgrade') == ''){
			global $sitepress;
			$current_lang_code= ICL_LANGUAGE_CODE;
			$default_language = $sitepress->get_default_language();	
			/* Insert wpml  icl_translations table*/
			$sitepress->set_element_language_details($upgradeid, $el_type='post_page', $upgradeid, $current_lang_code, $default_language );
			if(function_exists('wpml_insert_templ_post'))
				wpml_insert_templ_post($upgradeid,'page'); /* insert post in language */
		}
		 
		if(is_plugin_active('sitepress-multilingual-cms/sitepress.php') && get_option('tmpl_wpml_post_upgrade') == ''){
			update_option('tmpl_wpml_post_upgrade','inserted');
		}
...
if(is_admin()){
	...
	/* show renew option for amember product expires */
	if(get_option('templatic_licence_key_') && get_option('tmpl_hide_renew_button') != 1 && is_super_admin())
	{
		add_action('admin_notices', 'tmpl_notify_renew_product');
	}
}
 ...
 */
function tmpl_notify_tev_renew_product($plugin_file, $plugin_data,$status)
{
	...
	if ( (is_network_admin() || !is_multisite() ) && in_array(TEVOLUTION_SLUG,$tmpl_slug))
	{	
		$key = array_search(TEVOLUTION_SLUG, $tmpl_slug);
		echo '<tr class="plugin-update-tr tev-plugin-renew"><td colspan="3" class="plugin-update"><div class="update-message"> '.__('You Membership has been expired. You can Renew it from ','templatic-admin') .'<span style="color: #0073aa; cursor: pointer;" id="tmpl_redirect" target="_blank" data-key="'.$key.'" data-invoice-id="'.$tmpl_invoice_id[$key].'" data-invoice-item-id="'.$tmpl_invoice_item_id[$key].'" class="thickbox" title="Templatic renew">'.__('Here','templatic-admin').'</span></div></td></tr>';
	
	 }
	 add_action('admin_footer','tmpl_redirect_temp');
}

function tmpl_redirect_temp()
{
?>
	<script>
		jQuery(document).ready(function(){
			jQuery('#tmpl_redirect').click(function(){
				window.location = '//templatic.com/members/product_renew.php?item_id='+jQuery('#tmpl_redirect').attr("data-key")+'&invoice_id='+jQuery('#tmpl_redirect').attr("data-invoice-id")+'&invoice_item_id='+jQuery('#tmpl_redirect').attr("data-invoice-item-id");
			});
		});
	</script>
<?php
...
/* function for renew option for amember product expires */
if(!function_exists('tmpl_notify_renew_product'))
{
	function tmpl_notify_renew_product()
	{
		global $pagenow;
		if(isset($_REQUEST['tmpl_product_renew_hide']))
		{
			if (isset($_GET['_wpnonce']) || wp_verify_nonce($_GET['_wpnonce'], 'tmpl_hide_renew_button')) {
				update_option('tmpl_hide_renew_button',true);
		...
		$arg=array('method' => 'POST',
				 ...
				 'blocking' => true,
				 'headers' => array(),
				 'body' => array( 'licencekey' => get_option('templatic_licence_key_'),'action'=>'licensekey_verification','HTTP_HOST'=>$HTTP_HOST,'SERVER_ADDR'=>$SERVER_ADDR),
				 'user-agent' => 'WordPress/'. @$wp_version .'; '. home_url(),
				 'cookies' => array()
			);
		$warnning_message='';
		...
		{
			$response = wp_remote_get('https://templatic.com/members/membership_renew.php',$arg );
			if(!is_wp_error( $response )){
				$tmpl_show_renew_message = json_decode($response['body']);
				update_option('tmpl_renew_product',$tmpl_show_renew_message->success_message);
			}
		}
		
		$tmpl_response = explode(",",@$tmpl_show_renew_message->success_message);
		foreach($tmpl_response as $key=>$val)
		{
			$tmpl_renew_product_array = explode("=",$val);
			$tmpl_slug[] = $tmpl_renew_product_array[0];
		}
		$tmpl_renew_theme_plugin = 0;
		...
		for($i=0;$i<count($tmpl_slug);$i++)
		{
			if(( $tmpl_slug[(isset($tmpl_renew_product_array[$i]))?$tmpl_renew_product_array[$i]:''] == $theme->name || $tmpl_slug[(isset($tmpl_renew_product_array[$i]))?$tmpl_renew_product_array[$i]:''] == $theme->parent_theme ||  is_plugin_active($tmpl_slug[(isset($tmpl_renew_product_array[$i]))?$tmpl_renew_product_array[$i]:''])) )
			{
				$tmpl_renew_theme_plugin = 1;
			}
			if ( (is_network_admin() || !is_multisite() ) && @in_array(TEVOLUTION_EVENT_SLUG,$tmpl_slug) ) {	
				add_action('after_plugin_row_'.TEVOLUTION_SLUG, 'tmpl_notify_tev_renew_product',11 );
			}
		}
		
		$tmpl_renew_product = get_option('tmpl_renew_product');
		$tmpl_renew_product_array = explode(",",@$tmpl_show_renew_message->success_message);
		if ( (is_network_admin() || !is_multisite() ) && in_array(TEVOLUTION_SLUG,$tmpl_renew_product_array) ) {	
			add_action('after_plugin_row_'.TEVOLUTION_SLUG, 'tmpl_notify_tev_renew_product',11 );
		}
		
		
		if(@$tmpl_show_renew_message->success_message != 'Renew not available' && $tmpl_renew_theme_plugin )
		{
			$logo = sprintf('<img src="%s" alt="Templatic" style="height:30px" />', plugins_url('/Tevolution/tmplconnector/monetize/images/templatic.png', 'templatic-admin'));
			$message = sprintf(__('<p>You Membership has been expired. </p>
			<p>You can Renew it from %s %s</p>', 'templatic-admin')
				,'<a href="//templatic.com/members/member/" class="button-primary">' . __('Here', 'templatic-admin') . '</a>'
				, '<a href="' . admin_url(wp_nonce_url('index.php', 'templatic-admin') . '&tmpl_product_renew_hide','tmpl_hide_renew_button','tmpl_renew_nonce').'" class="button-secondary install-dismiss">Hide the message</a>'
			);
			//echo sprintf('<div class="update-nag below-h2 tev-cache-msg clearfix" style="width:%s">%s</div>',"60%", $logo . $message);
		}
	}
}
