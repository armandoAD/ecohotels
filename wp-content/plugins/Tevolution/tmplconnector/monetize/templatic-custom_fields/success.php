 ...
 */
$order_id = intval($_REQUEST['pid']);
global $page_title,$wpdb,$current_user;
...
{ 
	?><div id="content" class="large-9 small-12 columns "><?php _e('ERROR: Sorry, you are not allowed to view this post.','templatic');?></div><?php
}
else{
if(isset($_REQUEST['paydeltype']) && $_REQUEST['paydeltype']=='prebanktransfer' && @$_REQUEST['upgrade'] =='')
{
	...
	$search_array = array('[#to_name#]','[#payable_amt#]','[#information_details#]','[#site_name#]','[#admin_email#]','[#user_login#]','[#site_login_url#]','[#site_login_url_link#]');
	$replace_array = array($toEmailName,$sql_payable_amt,$transaction_details,$fromEmailName,get_option('admin_email'),$toEmailName,$store_login,$store_login_link);
	...
	if($tmpdata['post_default_status_paid']=='publish'){
		...
		if($payforfeatured_h == 1  && $payforfeatured_c == 1){
			update_post_meta(intval($_REQUEST['pid']), 'featured_c', 'c');
			update_post_meta(intval($_REQUEST['pid']), 'featured_h', 'h');
			update_post_meta(intval($_REQUEST['pid']), 'featured_type', 'both');			
		}elseif($payforfeatured_c == 1){
			update_post_meta(intval($_REQUEST['pid']), 'featured_c', 'c');
			update_post_meta(intval($_REQUEST['pid']), 'featured_type', 'c');
		}elseif($payforfeatured_h == 1){
			update_post_meta(intval($_REQUEST['pid']), 'featured_h', 'h');
			update_post_meta(intval($_REQUEST['pid']), 'featured_type', 'h');
		}else{
	...
	$replace_array = array($fromEmailName,$sql_payable_amt,$transaction_details,$fromEmailName,get_option('admin_email'),$toEmailName,$store_login,$store_login_link);
...
$amout=get_post_meta(intval($_REQUEST['pid']),'total_price',true);
if($amout=='0' || $amout==''){
	global $wpdb;
	$transaction_tabel = $wpdb->prefix."transactions";
	$tmpdata = get_option('templatic_settings');
	...
	if($_SESSION['custom_fields']['last_selected_pkg'])
	{
		...
		if(@$get_last_trans_status !='')
			$trans_status = $wpdb->query($wpdb->prepare("update $transaction_tabel SET status = ".$get_last_trans_status." where post_id = %d",$_REQUEST['pid']));
