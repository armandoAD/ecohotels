...
function new_googlemap_ajaxSearch(){
	...
	var post_city_id = jQuery('#map_sidebar #post_city_id').val();
	
	jQuery.ajax({
		url:tevolutionajaxUrl,
		type:'POST',
		async: true,
		data:'action=googlemap_initialize&post_city_id='+post_city_id+'&posttype='+post_type+'&categoryname='+categoryname+'&search_string=' + search_string,
...
function newgooglemap_initialize(map,posttype){
	...
	jQuery("input[name='posttype[]']").each(function() {/* post type loop*/
		...
		var checked_category1 = jQuery('#ajaxform input[name="categoryname[]"]:checked').length;
				
		if (checked_category1 > 0)
		{
			post_type=jQuery(this).val()+',';
			...
			jQuery.ajax({
				...
				type:'POST',
				async: true,
				data:'action=googlemap_initialize&post_city_id='+post_city_id+'&posttype='+post_type+'&categoryname='+categoryname,
				success:function(results){
					jQuery('.map_loading_div').css('display','none');
					googlemap(results);
					jsonData = jQuery.parseJSON(results);
			    ...
			    google.maps.event.addListener(mClusterer, 'clusterclick', function(cluster) {
					...
					if(map.getMapTypeId() == 'terrain'){
						...
						if(map.getZoom()==map_zoom){
							...
							//infowindow.closeBoxURL(closeimg)//(map, info);
							jQuery('img[src="http://maps.gstatic.com/mapfiles/api-3/images/mapcnt6.png"]').each(function() {
								jQuery(this).attr('src',closeimg);
                            });
