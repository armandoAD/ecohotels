...
function location_multicity_where($where){
	...
	/* latest post -page end */
		...
		if($current_cityinfo['city_id']!=''){
			if( isset($current_term) && ($current_term->taxonomy == CUSTOM_CATEGORY_TYPE_DEALS || $current_term->taxonomy == CUSTOM_TAG_TYPE_DEALS) && is_plugin_active('Tevolution-Deals/deals.php')){
