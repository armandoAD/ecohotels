...
if(isset($_POST['export_city_csv']) && isset($_POST['export_city']) && $_POST['export_city']=='1')
{
	...
	if($city_info){
	$is_zoom_cat = 0;$is_zoom_home = 0;
	 
	echo $header_top =  "City Id,Country Id,Zones Id,City Name,City Slug,Latitude,Longitude,Scall Factor,Is Zoom Home,Map Type,Post Type,Category ids,Is Default,Message,Color,Image,header color,Header image,Is Zoom Cat\r\n";
		foreach($city_info as $city){	
    if($city->is_zoom_home == 0 || $city->is_zoom_home == ''){ 	 $is_zoom_home =0;	 }else{	 $is_zoom_home = 1;	 }
	if($city->is_zoom_cat == 0 || $city->is_zoom_cat == ''){ 	 $is_zoom_cat = 0 ;}else{$is_zoom_cat = 1; }
