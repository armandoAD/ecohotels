...
<script  type="text/javascript"  src="//maps.googleapis.com/maps/api/js?v=3.5&sensor=false&libraries=places<?php echo $lang.$key;?>"></script>
...
jQuery(document).on("click",'input[name=map_type]', function(e){
   	initialize();
	geocode();
});
jQuery(document).on("change",'select[name=scaling_factor]', function(e){
   	initialize();
	geocode();
});
