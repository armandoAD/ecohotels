					...
					<div id="post-<?php the_ID(); ?>" class="<?php supreme_entry_class(); ?>">
					  <?php do_action( 'open_entry' ); 
						...
						<section class="entry-content">
							...
							<p class="download"> <a href="<?php echo wp_get_attachment_url(); ?>" title="<?php the_title_attribute(); ?>" rel="enclosure" type="<?php echo get_post_mime_type(); ?>"><?php printf( __( 'Download "%s";', 'templatic' ), the_title( '<span class="fn">', '</span>', false) ); ?></a> </p>
							<!-- .download -->
							<?php endif;
								...
								if ( !empty( $gallery ) )
									echo '<h3>' . __( 'Gallery', 'templatic' ) . '</h3>' . $gallery;
								}
								?>
						</section>
					  <!-- .entry-content -->
