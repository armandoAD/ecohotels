...
if(is_admin() && ($pagenow =='themes.php' || strstr($_SERVER['REQUEST_URI'],'update.php') || strstr($_SERVER['REQUEST_URI'],'update-core.php') || $pagenow =='update-core.php' || $pagenow =='post.php' || $pagenow =='edit.php'|| $pagenow =='admin-ajax.php' || trim($page) == trim('tmpl_theme_update')) && file_exists(get_template_directory().'/wp_theme_update.php') && !DOING_AJAX){
...
if(!function_exists('register_theme_settings_menu')){
	function register_theme_settings_menu() {
		...
		add_theme_page(__('Custom CSS Editor','templatic-admin'), __('Custom CSS Editor','templatic-admin'), 'edit_theme_options', 'templatic_custom_css_editor', 'templatic_custom_css_editor_settings');
	}
}
...
{
	$title = __('Custom CSS Editor','templatic-admin');
	...
	<h2><?php echo esc_html( $title ); ?></h2>
	 <p> <?php echo sprintf(__('You can customize the theme by entering CSS classes in this section. Enter only the classes you want to overwrite (not the whole style.css file). For details on using custom.css open %s.','templatic-admin'),'<a href="http://templatic.com/docs/using-custom-css-for-theme-customizations/">'.__('this article','templatic-admin').'</a>');?></p>
   
   <?php if(isset($_GET['updated']) && $_GET['updated'] == 'true'){ ?>
	    <div class="updated" id="message"><p><?php echo __('File edited successfully.','templatic-admin'); ?></p></div>
    <?php } ?>
	...
	<?php wp_nonce_field( 'edit-theme_' . $file . $stylesheet ); ?>
		...
		if(get_option('directory_custom_css'))
		{
			if ( is_writeable( $file ) ) :
				submit_button( __( 'Update File','templatic-admin' ), 'primary', 'submit', true );
			else : ?>
		<p><em><?php __('You need to make this file writable before you can save your changes. See <a href="http://codex.wordpress.org/Changing_File_Permissions">the Codex</a> for more information in directory root folder.','templatic-admin'); ?></em></p>
		<?php endif; 
		}
		else
		{
