...
function supreme_entry_published( $args ) {
	$args =  wp_parse_args( array( 'before' => __('On','templatic')." ", 'after' => ' ', 'format' => get_option( 'date_format' ) ), $args );
...
function supreme_entry_comments_link( $args ) {
	...
	$args = shortcode_atts( array( 'zero' => __( 'Leave a response', 'templatic' ), 'one' => apply_filters('comment_response_link',__( '%1$s Response', 'templatic' )), 'more' => __( '%1$s Responses', 'templatic' ), 'css_class' => 'comments-link', 'none' => '', 'before' => '', 'after' => '' ), $args );
	if ( 0 == $number && !comments_open() && !pings_open() ) {
		if ( $args['none'] )
			$comments_link = '<span class="' . esc_attr( $args['css_class'] ) . '">' . sprintf( $args['none'], number_format_i18n( $number ) ) . '</span>';
	}
	elseif ( 0 == $number )
		$comments_link = '<a class="' . esc_attr( $args['css_class'] ) . '" href="' . get_permalink() . '#respond" title="' . sprintf( esc_attr__( 'Comment on %1$s', 'templatic' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $args['zero'], number_format_i18n( $number ) ) . '</a> ';
	elseif ( 1 == $number )
		$comments_link = '<a class="' . esc_attr( $args['css_class'] ) . '" href="' . get_comments_link() . '" title="' . sprintf( esc_attr__( 'Comment on %1$s', 'templatic' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $args['one'], number_format_i18n( $number ) ) . '</a> ';
	elseif ( 1 < $number )
		$comments_link = '<a class="' . esc_attr( $args['css_class'] ) . '" href="' . get_comments_link() . '" title="' . sprintf( esc_attr__( 'Comment on %1$s', 'templatic' ), the_title_attribute( 'echo=0' ) ) . '">' . sprintf( $args['more'], number_format_i18n( $number ) ) . '</a> ';
	if ( $comments_link )
...
function supreme_entry_author( $args ) {
	$args = wp_parse_args( array( 'before' => __( 'Published by', 'templatic' )." ", 'after' => ' ' ), $args );
...
function supreme_entry_title( $args ) {
	...
	if ( empty( $title ) && !is_single() && !is_page() && !is_404() && $post->post_title !=''){
		$title = "<{$tag} class='{$class}'><a href='" . get_permalink() . "'>" . __( '(Untitled)', 'templatic' ) . "</a></{$tag}>";
	}	
...
function supreme_entry_permalink( $args ) {
	$args = wp_parse_args( array( 'before' => '', 'after' => '' ), $args );
	echo $args['before'] . '<a href="' . esc_url( get_permalink() ) . '" class="permalink">' . __( 'Permalink', 'templatic' ) . '</a>' . $args['after'];
}
