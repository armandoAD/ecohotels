...
function tmpl_autoinstall()
{
	...
	global $wpdb;
		...
		/* help links */
		$menu_msg1 = "<ul>";
		$menu_msg1 .= "<li><a href='".site_url("/wp-admin/post-new.php?post_type=listing")."'>".__('Add a listing','templatic-admin')."</a></li>";
			...
			if(isset($_REQUEST['dummy_insert']) && $_REQUEST['dummy_insert']){
				...
				if(!$nav_menus){
					...
					if(!in_array('primary',$navmenu)){
						...
						$submit_listing_id = $wpdb->get_row($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE $wpdb->posts.post_name = 'submit-listing' and $wpdb->posts.post_type = 'page'"));
					...
					if(!in_array('secondary',$navmenu)){
						...
						/*Get people page id */
						$people_id = $wpdb->get_row($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE $wpdb->posts.post_name = 'people' and $wpdb->posts.post_type = 'page'"));
...
function tmpl_delete_dummy_data()
{
	...
	$pids_info = $wpdb->get_results($wpdb->prepare($pids_sql));
...
function set_page_info_autorun($pages_array,$page_info_arr)
{
	...
	{ 
		...
		$post_count = $wpdb->get_var($wpdb->prepare("SELECT count(ID) FROM $wpdb->posts where post_title like \"$post_title\" and post_type='page' and post_status in ('publish','draft')"));
		if(!$post_count)
		{
			$post_info_arr = array();
			$catids_arr = array();
