...
function tmpl_load_populer_post()
{
	...
	$posthtml = '';		
	$start = sanitize_text_field($_REQUEST['limitarr'][0]);
	$end = sanitize_text_field($_REQUEST['limitarr'][1]);
	$total = sanitize_text_field($_REQUEST['limitarr'][2]);
	$post_type = sanitize_text_field($_REQUEST['limitarr'][3]);
	$num=sanitize_text_field($_REQUEST['limitarr'][4]);
	$popular_per=sanitize_text_field($_REQUEST['limitarr'][5]);
	$number=sanitize_text_field($_REQUEST['limitarr'][6]);
	...
	if($popular_post_query):
		...
		while ($popular_post_query->have_posts()) : $popular_post_query->the_post();
			...
			$comments = $post->comment_count.' '.__('Comment','templatic');
			if($post->comment_count > 1 || $post->comment_count ==0){
				$comments = $post->comment_count.' '.__('Comments','templatic');
			}
			...
			$fv = short_time_diff($du, current_time('timestamp')). " " . __('ago','templatic');
			$fv = sprintf(__('%s','templatic'),$fv);
			if($popular_per == 'views' || $popular_per == 'dailyviews'){
				...
				if($views <= 1){ $views = $views." ".__('view','templatic'); }else{ 
				
					$views = $views." ".__('views','templatic');  
	...
	<p>
	  <?php _e('No Popular post fond.','templatic');exit;?>
	</p>
	<?php
	endif;
	wp_reset_query();	
...
function short_time_diff( $from, $to = '' ) {
    ...
    $replace = array(
        'min' => __('min','templatic'),
        'mins' => __('mins','templatic'),
        'hour' => __('hours','templatic'),
        'hours' => __('hours','templatic'),
        'day' => __('day','templatic'),
        'days' => __('days','templatic'),
        'week' => __('week','templatic'),
        'weeks' => __('weeks','templatic'),
        'month' => __('month','templatic'),
        'months' => __('months','templatic'),
        'year' => __('year','templatic'),
        'years' => __('years','templatic'),
    );
...
function product_add_custom_box() {
    ...
    if(is_plugin_active('woocommerce-bookings/woocommmerce-bookings.php'))
	{
	    foreach ($post_typearr as $screen) {
		   add_meta_box('listproduct_sectionid', __( 'Select Your Services & Products', 'templatic' ),'listproduct_custom_boxes', $screen,'side');
	    }
    }
}
	...
	if ( $products->have_posts() ) : ?>
		...
		<div id="posttype-event-listing" class="posttypediv">		
			<div id="listing-event-all" >
				...
				<ul id="listing_product_checklist" data-wp-lists="list:<?php echo 'product'?>" class="categorychecklist form-no-clear">
				<select class="custom_posttypes" name="product_for_listing" id="product_for_listing">
					 <option value=""><?php _e('Select Product','templatic');?></option>
					<?php
					global $post;
					$booking_event = $post;
					while ( $products->have_posts() ) : $products->the_post();	
						...
						if(!empty($product_for_listing) && in_array($event_id,$product_for_listing) && $event_id !='' || $event_id == get_post_meta($_REQUEST['post'],'product_for_listing',true)  ){ $selected = 'selected=selected'; }else{ $selected='';}
...
function product_save_postdata($post_id){
	...
	if(isset($_POST['product_for_listing']) && $_POST['product_for_listing']!='')
	{
		update_post_meta($post_id,'product_for_listing',wp_kses_post(trim($_POST['product_for_listing']))); /* booked tickets*/
	}
	/* if no event is selected then make meta value blank */
	else
	{
		update_post_meta($post_id,'product_for_listing',''); /* booked tickets*/
	}
}
