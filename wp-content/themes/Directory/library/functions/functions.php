...
function directory_pagenavi_plugin($before = '', $after = '') {
    ...
    $pagenavi_options['next_text'] = '<strong class="next page-numbers">' . __('Next', 'templatic') . '<span class="meta-nav">&rarr;</span></strong>';
    $pagenavi_options['prev_text'] = '<strong class="prev page-numbers"><span class="meta-nav">&larr;</span> ' . __('Previous', 'templatic') . '</strong>';
...
function search_form_for_404_display($searchform) {
    $searchform = '<div class="404_search">
			<form role="search" method="get" class="search-form" action="' . esc_url(home_url('/')) . '">
			   <input type="text" size="100" placeholder="' . __('Looking for....', 'templatic') . '" class="searchpost " id="s" name="s" value="" autocomplete="off">
               <input type="submit" alt="" class="sgo" value="' . __('Search', 'templatic') . '" />
          </form></div>';
    return $searchform;
}
...
function directory_add_custom_post_field($edit_post) {
    ...
    ?>
    <tr>
        <th valign="top"><label for="marker" class="form-textarea-label"><?php echo __('Category map markers', 'templatic-admin'); ?></label></th>
        <td>
            <div class="input-switch">
                <input type="checkbox" id="taxonomy_marker" name="taxonomy_marker" value="enable" <?php if ($taxonomy != '' && $tevolution_taxonomy_marker[$taxonomy] == 'enable') {
        echo 'checked';
    } ?> />&nbsp;<label for="taxonomy_marker"><?php echo __('Enable', 'templatic-admin'); ?></label>
            </div>
            <p class="description"><?php echo __('Enabling this will allow you to assign a custom icon(marker) for categories within this post type. The marker can be set while creating the category.', 'templatic-admin'); ?></p>
        </td>
    </tr>
    <?php
...
function templatic_comments_number($zero = false, $one = false, $more = false, $deprecated = '') {
    ...
    if ($number > 1)
        $output = ( false === $more ) ? number_format_i18n($number) . " " . __('Comments', 'templatic') : number_format_i18n($number) . " " . $more;
    elseif ($number == 0)
        $output = ( false === $zero ) ? __('No Comments', 'templatic') : $zero;
    else // must be one
        $output = ( false === $one ) ? __('1 Comment', 'templatic') : $one;
...
function supreme_default_theme_settings() {
    ...
    if (is_array($supports[0]) && in_array('footer', $supports[0]) && current_theme_supports('supreme-core-shortcodes')) {
        ...
        if (is_child_theme())
            $settings['footer_insert'] = '<p class="copyright">' . __('Copyright &#169; [the-year] [site-link].', 'templatic') . '</p>' . "\n\n" . '<p class="credit">' . __('Powered by [wp-link], [theme-link], and [child-link].', 'templatic') . '</p>';
        /* If no child theme is active, leave out the [child-link] shortcode. */
        else
            $settings['footer_insert'] = '<p class="copyright">' . __('Copyright &#169; [the-year] [site-link].', 'templatic') . '</p>' . "\n\n" . '<p class="credit">' . __('Powered by [wp-link] and [theme-link].', 'templatic') . '</p>';
    }
...
if (!function_exists('the_content_limit')) {
    ...
    function the_content_limit($max_char, $more_link_text = '', $stripteaser = true, $more_file = '') {
        global $post;
		$post_id=$post->ID;
        $more_link_text = ($more_link_text != '') ? $more_link_text : __('Read More &raquo;', 'templatic');
        $content = get_the_content();
        $content = apply_filters('the_content', $content);
        $content = strip_tags($content);
        $content = substr($content, 0, $max_char);
        $content = substr($content, 0, strrpos($content, " "));
        $more_link_text = '<a href="' . get_permalink($post_id) . '">' . $more_link_text . '</a>';
...
function supreme_loop_navigation($post) {
    ...
    <?php elseif (is_singular('post')) : ?>
        ...
        <!-- .loop-nav -->
    <?php elseif (!is_singular() && current_theme_supports('loop-pagination')) : loop_pagination(array('prev_text' => '<strong class="prev page-numbers">' . __('Previous', 'templatic') . '</strong> ', 'next_text' => ' <strong class="next page-numbers">' . __('Next', 'templatic') . '</strong>')); ?>
    <?php elseif (!is_singular() && $nav = get_posts_nav_link(array('sep' => '', 'prelabel' => '<span class="previous"><span class="meta-nav">&larr;</span> ' . __('Previous', 'templatic') . '</span>', 'nxtlabel' => '<span class="next">' . __('Next', 'templatic') . ' &rarr;</span>'))) : echo "qwe"; ?>
...
function supreme_post_gallery($post) {
    ...
    ?>
        ...
        <?php
        for ($im = 0; $im < count($post_main_image); $im++):
    ...
    function supreme_view_count($content) {
        ...
        if (is_single() || is_singular() && !is_page()) {
            ...
            if (user_single_post_visit_count($post->ID) == 1) {
                $time = ' ' . __("time", 'templatic');
            } else {
                $time = ' ' . __("times", 'templatic');
            }

            if (user_single_post_visit_count_daily($post->ID) == 1) {
                $visit = ' ' . __("Visit", 'templatic');
            } else {
                $visit = ' ' . __("Visits", 'templatic');
            }
            $custom_content.="<p><span class='view_counter'>" . __("Visited", 'templatic') . '<b>' . sprintf(__(' %s', 'templatic'), user_single_post_visit_count($post->ID)) . $time . '</b>';
            $custom_content.= $sep . ' <b>' . user_single_post_visit_count_daily($post->ID) . ' ' . $visit . "</b>" . ' ' . __("today", 'templatic') . "</span></p>";
...
function supreme_primary_navigation() {
    ...
    if (has_nav_menu('primary') || isset($_REQUEST['ptype']) || isset($_REQUEST['trans_id'])) :
        ...
        <?php _e('Menu', 'templatic'); ?>
                </div>
                <!-- #menu-primary-title -->
        <?php
        do_action('open_menu_primary'); // supreme_open_menu_primary 
        ...
        <?php
        do_action('after_menu_primary'); // supreme_after_menu_primary
    endif;
}
...
function supreme_home_banner_sidebar() {
    ...
    if ((is_home() || is_front_page() || $current_page_template == 'page-templates/front-page.php') && ($queried_object->ID != $as_posts_page))
...
function supreme_theme_layouts($strings) {
    ...
    $strings = array(
        'default' => __('Default', 'templatic-admin'),
        '1c' => __('One Column', 'templatic-admin'),
        '2c-l' => __('Two Columns, Left', 'templatic-admin'),
        '2c-r' => __('Two Columns, Right', 'templatic-admin')
    );
    return $strings;
}
        ...
        if (is_single() && ($post->post_type != 'product_variation' )) {
            ...
            if (@$settings['facebook_share_detail_page'] || @$settings['google_share_detail_page'] || @$settings['twitter_share_detail_page'] || @$settings['pintrest_detail_page']) {
                ...
                if (@$settings['facebook_share_detail_page'] == 1) {
                    ...
                    <?php _e('Facebook Share.', 'templatic'); ?>
                    </a>
                    <?php
                }
...
function crop_popular_post_thumb_image() {
    return get_the_image(array('echo' => false, 'size' => 'popular_post-thumbnail', 'height' => 60, 'width' => 60, 'default_image' => "//placehold.it/60x60"));
}
...
if (!is_plugin_active('wordpress-seo/wp-seo.php')) {
    ...
    /** To Hide Extra Meta Keywords on activation of Yoast Plugin **/
...
function supreme_author_biography_($post) {
    ...
    if (is_single() && $supreme_author_bio_posts) {
        ?>
        <div class="entry-author-meta"> <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')); ?>" title="<?php echo esc_attr(get_the_author_meta('display_name')); ?>" class="avatar-frame"><?php
        if (get_user_meta(get_the_author_meta('ID'), 'profile_photo', true) != "") {
            echo '<img class="avatar" alt="avatar" src="' . get_user_meta(get_the_author_meta('ID'), 'profile_photo', true) . '" alt="' . esc_attr(get_the_author_meta('display_name')) . '" title="' . esc_attr(get_the_author_meta('display_name')) . '" width="60" height="60"/>';
        } else {
            echo get_avatar(get_the_author_meta('ID'), '60', '', '');
