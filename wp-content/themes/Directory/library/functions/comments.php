...
function supreme_comment_form_args( $args ) {
	...
	$fields = array(
		...
		'url' => '<p class="form-url"><label for="url">' . __( 'Website', 'templatic' ) . '</label><input type="text" class="text-input" name="url" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="40" /></p>'
	);
	...
	$args = array(
		...
		'must_log_in' => '<p class="alert">' . __( 'You must be','templatic').' <a href="'.$login_url.'" title="'.__('Log in','templatic').'">'.__('logged in','templatic').'</a>'.__(' to post a comment.', 'templatic' ) . '</p><!-- .alert -->',
		'logged_in_as' => '<p class="log-in-out">' . __( 'Logged in as', 'templatic' )." ".'<a href="'.get_author_posts_url( get_current_user_id() ).'" title="'. esc_attr( $user_identity ).'">'. esc_attr( $user_identity ).'</a>. <a href="' . wp_logout_url( get_permalink() ) . '" title="' .__( 'Log out of this account', 'templatic' ). '">' . __( 'Log out &raquo;', 'templatic' ) . '</a></p><!-- .log-in-out -->',
		'comment_notes_before' => '',
		'comment_notes_after' => '',
		'id_form' => 'commentform',
		'id_submit' => 'submit',
		'title_reply' => (get_post_type()=='post')? __( 'Leave a Reply', 'templatic' ):__( 'Add a Review', 'templatic' ) ,
		'title_reply_to' => (get_post_type()=='post')?__( 'Leave a Reply to %s', 'templatic' ) :__( 'Add a Review to %s', 'templatic' ) ,
		'cancel_reply_link' => __( 'Click here to cancel reply.', 'templatic' ),
		'label_submit' => __( 'Post Comment', 'templatic' ),
