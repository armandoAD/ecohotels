...
function supreme_document_title() {
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date () ) {
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$doctitle = __( 'Archive for ', 'templatic' ).get_the_time( __( 'g:i a', 'templatic' ) );
			elseif ( get_query_var( 'minute' ) )
				$doctitle = __( 'Archive for minute ', 'templatic' ).get_the_time( __( 'i', 'templatic' ) );
			elseif ( get_query_var( 'hour' ) )
				$doctitle = __( 'Archive for ', 'templatic' ).get_the_time( __( 'g a', 'templatic' ) );
			elseif ( is_day() )
				$doctitle = __( 'Archive for ', 'templatic' ).get_the_time( __( 'F jS, Y', 'templatic' ) );
			elseif ( get_query_var( 'w' ) )
				$doctitle = __( 'Archive for week ', 'templatic' )." ".get_the_time( __( 'W', 'templatic' ) ).__('of','templatic')." ".get_the_time( __( 'Y', 'templatic' ) );
			elseif ( is_month() )
				$doctitle = __( 'Archive for ', 'templatic' ).single_month_title( ' ', false);
			elseif ( is_year() )
				$doctitle = __( 'Archive for ', 'templatic' ).get_the_time( __( 'Y', 'templatic' ) );
		}
		/* For any other archives. */
		else {
			$doctitle = __( 'Archives', 'templatic' );
		}
	}
	/* If viewing a search results page. */
	elseif ( is_search() )
		$doctitle = __( 'Search results for ', 'templatic' )."&quot;".esc_attr( get_search_query() )."&quot;";
	/* If viewing a 404 not found page. */
	elseif ( is_404() )
		$doctitle = __( '404 Not Found', 'templatic' );
	/* If the current page is a paged page. */
	if ( ( ( $page = $wp_query->get( 'paged' ) ) || ( $page = $wp_query->get( 'page' ) ) ) && $page > 1 )
		$doctitle = sprintf( __( '%1$s Page %2$s', 'templatic' ), $doctitle . $separator, number_format_i18n( $page ) );
	...
	if((isset($_REQUEST['pmethod']) && $_REQUEST['pmethod'] !='') || (isset($_REQUEST['paydeltype']) && $_REQUEST['paydeltype']!='')){
		...
		$doctitle = __('Submitted Successfully','templatic');
	}
