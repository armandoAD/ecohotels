...
/**
 * Filters 'gettext' to change the translations used for the 'templatic' textdomain.  This filter makes it possible 
 * for the theme's MO file to translate the framework's text strings.
 */
function supreme_gettext( $translated, $text, $domain ) {
	/* Check if 'templatic' is the current textdomain, there's no mofile for it, and the theme has a mofile. */
	if ( 'templatic' == $domain && !supreme_is_textdomain_loaded( 'templatic' ) && supreme_is_textdomain_loaded( supreme_get_parent_textdomain() ) ) {
...
function supreme_extensions_gettext( $translated, $text, $domain ) {
	...
	if ( in_array( $domain, array( 'breadcrumb-trail', 'custom-field-series', 'post-stylesheets', 'theme-layouts' ) ) ) {
		...
		if ( current_theme_supports( $domain ) ) {
			...
			if ( supreme_is_textdomain_loaded( 'templatic' ) )
				$translations = &get_translations_for_domain( 'templatic' );
