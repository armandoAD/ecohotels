...
function tmpl_get_remote_verison(){
	...
	{ 
		...
		{		
			...
			{
				$warnning_message= sprintf(__("Update was unable to continue because %s theme doesn't seem to be associated with your Templatic account.",'templatic-admin'),$th_name)." ".__("Please",'templatic-admin')." <a href='http://templatic.com/contact/' target='_blank'>".__("contact us",'templatic-admin')."</a> ".__("if this message is showing in error.",'templatic-admin');
			}			
...
<div class='wrap templatic_login'>
  <?php if($flag ==1){ ?>
  <div id="update-nag">
    <p style=" clear:both;"> <?php echo sprintf(__('The new version of %s is available.','templatic-admin'),$theme_name); ?></p>
    <p>
      <?php echo __('you can update to the latest version automatically , or download the latest version of the theme.','templatic-admin'); ?>
    </p>
    <p><span style="color:red; font-weight:bold;">
      <?php echo __('Warning','templatic-admin'); ?>
      : </span>
      <?php echo __('Remember that the updates will replace all your changes so please keep backup of your changes.','templatic-admin'); ?>
    </p>
    <a class="button-secondary" href="http://templatic.com/members/member" target="blank">
    <?php echo __('Download latest Version','templatic-admin'); ?>
    </a> </div>
  <?php } ?>
  <!-- Logo -->
  ...
  <?php
		if($flag ==1){
			...
			{?>
  <div class='error'>
    <p><strong><?php echo sprintf(__('%s','templatic-admin'), $warnning_message);?></strong></p>
  </div>
  <?php
			  }
			if(!isset($successfull_login) && $successfull_login!=1):?>
  <p class="info"> <?php echo  sprintf(__('Enter your templatic login credentials to update your %s theme to the latest version.','templatic-admin'),$th_name);?> </p>
  <form action="<?php echo site_url()."/wp-admin/admin.php?page=tmpl_theme_update";?>" name="" method="post">
    <table>
      <tr>
        <td><label>
            <?php echo __('User Name', 'templatic-admin')?>
          </label></td>
        <td><input type="text" name="templatic_username"  /></td>
      </tr>
      <tr>
        <td><label>
            <?php echo __('Password', 'templatic-admin')?>
          </label></td>
      ...
      <tr>
        ...
        <td><a title="Close" id="TB_closeWindowButton" href="#" class="button-secondary">
          <?php echo __('Cancel','templatic-admin'); ?>
          </a></td>
      </tr>
    </table>
  </form>
  <?php else:								
				 ...
				 echo $th_name.' - theme <a id="TB_closeWindowButton" class="update_btn" href="'.$download.'" target="_parent" class="button-secondary">'.__('Update Now','templatic-admin').'</a>';
			 endif;
...
<?php
	if($flag == 0){
		echo '<h3>'.__('You have the latest version of the ','templatic-admin').$th_name.'</h3>';
