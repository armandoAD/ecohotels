  ...
  <div class="author_content">
    <div class="agent_biodata">
      ...
      <?php endif;//finish check current author user url		
				...
				if($pkg_type == 2){
					echo "<p>"; 
					_e('You have subscribed to ','templatic')."<strong>".$package_name."</strong> ";
					_e(' which allows you to submit ','templatic')." <strong>".$limit_no_post."</strong> "." events";
					$msg ='';
					if($remaining >0){
						_e(" You have submitted ",'templatic')."<strong>".$submited." </strong>";
						_e("events till now, go and submit remaining ",'templatic')."<strong>".$remaining." </strong>";
	
					}else{
						_e(" and you have already submitted.",'templatic'); echo ".<strong>".$limit_no_post."</strong> "; _e("to continue the listing Click on add/submit events.",'templatic');
					}
...
<div class="loop-meta extra-search-criteria-title">
  ...
  $search_text = (get_search_query() != ' ') ? get_search_query() : $_REQUEST['location'];
   _e('Search results for','templatic');echo ' "'.$search_text.'"'; ?></h1>
  ...
  <?php
	...
	if(isset($radius)){
	?>
    <p><?php _e( 'You are browsing the search results for ', 'templatic' )."<strong>".esc_attr( get_search_query() )."</strong> ".'<span>'. $radius.'</span>'; ?></p>
	<?php } ?>
  </div>
  <!-- .loop-description -->
</div>
...
<div class="loop-meta">
  <?php if ( is_day() ) : ?>
  <h1 class="loop-title"><?php _e( 'Daily Archives: ', 'templatic' ). '<span>' . get_the_date() . '</span>';?></h1>
  <div class="loop-description">
    <p><?php _e( 'You are browsing the site archives by.', 'templatic' ). '<span>' . get_the_date() . '</span>'; ?></p>
  </div>
  <!-- .loop-description -->
  <?php elseif ( is_month() ) :?>
  <h1 class="loop-title"><?php _e( 'Monthly Archives:', 'templatic' ). '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'templatic' ) ) . '</span>';?></h1>
  <div class="loop-description">
    <p><?php _e( 'You are browsing the site archives by.', 'templatic' ). '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'templatic' ) ) . '</span>'; ?></p>
  </div>
  <!-- .loop-description -->
  <?php elseif ( is_year() ) :?>
  <h1 class="loop-title"><?php _e( 'Yearly Archives:', 'templatic' ).'<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'templatic' ) ) . '</span>' ; ?></h1>
  <div class="loop-description">
    <p><?php _e('You are browsing the site archives by.', 'templatic' ).'<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'templatic' ) ) . '</span>' ; ?></p>
  </div>
  <!-- .loop-description -->
  <?php endif;?>
</div>
...
<div class="loop-meta">
  ...
  <div class="loop-description">
    <p>
      <?php _e( 'You are browsing the site archives.', 'templatic' ); ?>
    </p>
  </div>
  <!-- .loop-description -->
</div>
