    ...
    class WPUpdates_Supreme_Updater {
		...
		function __construct( $api_url,  $theme_slug ) {
			...
			if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
			  $page_text = __("Oops!! Something went wrong.<br/>Please try again or <a href='http://www.templatic.com'>contact us</a>.",'templatic-admin');
			}
			else{
		...
		function supreme_child_update_nag($transient) {
			...
			{	
				echo '<div id="update-nag">';
				 $new_version = version_compare($supreme_version, $remote_version, '<') ? sprintf(__('There is a new version of %s available.', 'templatic-admin') ,$theme_name) : '';		
				 ...
				 $new_version .= ' <a class="thickbox" title="Directory" href="'.$changelog.'">'. sprintf(__('View version %s Details', 'templatic-admin'), $remote_version) . '</a>. Or ' ;
					...
					echo '<tr class="plugin-update-tr">
							<td colspan="3" class="plugin-update">
								<div class="update-message">' . $new_version . '<a href="'.$ajax_url.'" title="Directory Update">'.__( 'update now','templatic-admin').'</a></div>
							</td>
						 </tr>';
				echo '</div>';
			}
		...
		function supreme_theme_row()
		{
			...
			if (version_compare($theme_version, $remote_version, '<') && $theme_version!='')
			{	
			   	$new_version = version_compare($supreme_version, $remote_version, '<') ? __('There is a new version of Directory available.', 'templatic-admin') : '';
				$changelog = esc_url( add_query_arg( array( 'slug' => 'changelog', 'action' => 'changelog' , '_ajax_nonce' => wp_create_nonce( 'changelog' ), 'TB_iframe' => true ,'width'=>500,'height'=>400), admin_url( 'admin-ajax.php' ) ) );
				 $new_version .= ' <a class="thickbox" title="Directory" href="'.$changelog.'">'. sprintf(__('View version %s Details', 'templatic-admin'), $remote_version) . '</a>. Or ' ;
				...
				echo '</tr><tr class="plugin-update-tr"><td colspan="3" class="plugin-update"><div class="update-message">' . $new_version .'<a href='.$ajax_url.' class="thickbox" title="Directory Update">'.__( 'update now','templatic-admin').'</a></div></td>';
