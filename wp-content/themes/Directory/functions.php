...
function supreme_theme_setup() {
	...
	if(is_admin() && is_writable(WP_CONTENT_DIR."/plugins") && is_readable(get_template_directory()) && ($pagenow=='themes.php' || $pagenow=='theme-install.php') ){
		...
		if ( is_admin() && isset($_GET['activated'] ) && $pagenow == 'themes.php' && $theme_name == 'Directory 2') {
...
function dir_one_click_install(){
	echo "<div id='ajax-notification' class='updated'><p><span style='color:red;'>AUTHENTICATION REQUIRED:</span>".__('Your server does not allow automated plugin activation so you will have to activate the plugins manually one by one.','templatic-admin')."</p>  </div>";
}
...
function tmpl_theme_css_scripts() {
	...
	wp_enqueue_style( 'directory-css', get_stylesheet_uri(),array('templatic-directory-css','templatic-admin-css'));
	...
	{	
		if( file_exists(get_stylesheet_directory().'/theme-style.css'))
		{
			wp_enqueue_style( 'tmpl_childtheme_view', trailingslashit ( get_stylesheet_directory_uri() ) . 'theme-style.css', array('tmpl_dir_css') );
		}
	}
	...
	if ( ((isset($supreme2_theme_settings['rtlcss']) && $supreme2_theme_settings['rtlcss']==1) || is_rtl())&& !is_admin() ) { 
...
function comment_form_defaults_comment_title( $arg ) {
	$arg['comment_field'] = '<p class="comment-form-comment"><label for="comment">'.__('Review','templatic').'</label> <textarea placeholder="'.__('This is the comment area from where you can give reivew and ratings.','templatic').'" aria-required="true" rows="8" cols="45" name="comment" id="comment"></textarea></p>';
	return $arg;
}
/*
...
function comment_form_defaults( $arg ) {
	...
	if($post->post_type != 'post')
		$arg['label_submit'] = __('Post Review','templatic');
	return $arg;
}
...
function templatic_wp_head() {
	...
	if($supreme2_theme_settings['rtlcss'] == 1 || is_rtl()):
...
if(!function_exists('tmpl_directory_fb_metatags')){
	function tmpl_directory_fb_metatags(){
		...
		/* Condition to add meta for gallery images if no feature and no content image Start */
		else{
...
function tmpl_directory_twitter_metatags(){
	global $post;
		if(is_single() && !is_plugin_active('wordpress-seo/wp-seo.php')){
			...
			<meta name="twitter:description" content="<?php echo substr(strip_tags($post->post_content),0,100); ?>" />
			<?php
			...
			if(function_exists('bdw_get_images_plugin'))
			{
				$large_image_url = bdw_get_images_plugin($post->ID,'large');
				if (!empty($large_image_url) ) { ?>
					<meta name="twitter:image" content="<?php echo $large_image_url[0]['file']; ?>" />
					<?php
				}
			} 
...
function supreme_get_sidebars() {
	...
	$sidebars = array(
		'header' => array(
			'name' =>	apply_filters('supreme_header_right_title',__( 'Header', 'templatic-admin' )),
			'description' =>	apply_filters('supreme_header_right_description',__( "The area is located on the right side of your header (between primary and secondary navigation).", 'templatic-admin' )),
		),
		'secondary_navigation_right' => array(
			'name' =>	__( 'Secondary Navigation', 'templatic-admin' ),
			'description' =>	__( 'Widgets placed inside this area will appear on the right side of your secondary navigation bar (the one below the logo). The simple "Search" widget works best here.', 'templatic-admin' ),
		),
		'home-page-banner' => array(
			'name' =>	apply_filters('supreme_home_page_banner_title',__( 'Homepage Banner', 'templatic-admin' )),
			'description' =>	apply_filters('supreme_home_page_banner_description',__( "This area usually displays the big slider or the map. It's located between the header and the Homepage - Main Content area.", 'templatic-admin' )),
		),	
		...
		'after-singular' => array(
			'name' =>	apply_filters('tmpl_detail_page_below_content',__( 'Detail Pages - Below Content','templatic-admin' )),
                              ...
                              jQuery('#page_template').change(function() {
                                        if(this.value == 'page-templates/advance-search.php' || this.value == 'page-templates/contact-us.php'){
                                                  if(this.value == 'page-templates/advance-search.php'){
                                                            var with_div_output = '<div id="page_template_notification" style="color:red;"><p>Note: Add Advance search widget to Advance search content area from Appearence-> <a href="'+pageTemplateWidgetUrl+'"><?php echo __('widgets','templatic-admin'); ?></a></p></div>';
                                                            var without_div_output = '<p><?php echo __('Note: Add Advance search widget to Advance search content area from Appearence-> ','templatic-admin'); ?>  <a href="'+pageTemplateWidgetUrl+'"><?php echo __('widgets','templatic-admin'); ?></a></p>';
                                                  }else{
                                                            var with_div_output = '<div id="page_template_notification" style="color:red;"><p><?php echo __('Note: Add contact us widget to contact page - main content area from Appearence-> ','templatic-admin'); ?><a href="'+pageTemplateWidgetUrl+'"><?php echo __('widgets','templatic-admin'); ?></a></p></div>';
                                                            var without_div_output = '<p><?php echo __('Note: Add contact us widget to contact page - main content area from Appearence-> ','templatic-admin'); ?> <a href="'+pageTemplateWidgetUrl+'"><?php echo __('widgets','templatic-admin'); ?></a></p>';
                                                  }
