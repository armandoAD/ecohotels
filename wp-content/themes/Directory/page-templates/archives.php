...
<section id="content" class="multiple large-9 small-12 columns">
  ...
  do_action( 'open_content' );  ?>
    ...
    <?php 
		...
		$years = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT MONTH(post_date) AS month, YEAR(post_date) as year
			FROM $wpdb->posts WHERE post_status = 'publish' and post_date <= now( ) and 
			post_type = '%s' ORDER BY post_date DESC",$templatic_catelog_post_type));
				...
				<div class="arclist">
				  ...
				  <ul>
					...
					<li> <a href="<?php the_permalink() ?>">
					  ...
					  <?php _e('on','templatic');?>
					  <?php the_time(__(get_option('date_format'),'templatic')) ?>
