...
if((!is_plugin_active('Tevolution/templatic.php') || !is_plugin_active('Tevolution-'.$related_addon.'/'.strtolower($related_addon).'.php')) && is_admin() && 'themes.php' == $pagenow ){
	/*  action show notification when tevolution not activated. */
	add_action("admin_notices", "tmpl_splendor_activate_eco_plugin"); 
}else{
		if(false==get_option( 'hide_ajax_notification' ) && strstr($_SERVER['REQUEST_URI'],'themes.php') && (!isset($_REQUEST['page']))) {
			add_action("admin_notices", "tmpl_splendor_autoinstall"); 
		}
...
*/
function tmpl_splendor_activate_eco_plugin(){
	global $pagenow;
	$url = home_url().'/wp-admin/plugins.php';
	tmpl_splendor_add_css_to_admin();
	$current_system = '';
	$related_addon= wp_get_theme();
	...
	if(!is_plugin_active('Tevolution/templatic.php') && is_admin() ){
		$current_system = "<a id='templatic_plugin' href=".$url." style='color:#21759B'>".__('Tevolution','templatic-admin')."</a>";
	}	
	if(!is_plugin_active('Tevolution-'.$related_addon.'/'.strtolower($related_addon).'.php') && is_admin() ){
		if($current_system != '')
			$current_system .= __(' and ', 'templatic-admin');
		$current_system .= '<a id="booking_plugin" href="'.$url.'" style="color:#21759B">'.__('Tevolution - ','templatic-admin').$related_addon.'</a>';
	}
...
?>
  ...
  <?php echo sprintf(__('Thanks for choosing templatic themes, It seems to want Tevolution-Directory activated first before the warning and verification request go away. Please download and activate %s addons to get started with %s website.','templatic-admin'),$current_system,'<span style="color:#000">'. @wp_get_theme().'</span>');?>
  </span> 
  </div>
<?php 	
	endif;
...
add_action('admin_notices','tmpl_splendor_add_css_to_admin');
function tmpl_splendor_add_css_to_admin(){
...
function tmpl_splendor_autoinstall()
{
	global $wpdb;
	...
	global $wpdb;
		...
		/* help links */
		$menu_msg1 = "<ul>";
		$menu_msg1 .= "<li><a href='".site_url("/wp-admin/post-new.php?post_type=listing")."'>".__('Add a Hotel','templatic-admin')."</a></li>";
		$menu_msg1 .= "<li><a href='".site_url("/wp-admin/user-new.php")."'>".__('Add Users','templatic-admin')."</a></li>";
		...
		/* theme message */	
		$dummy_theme_message .='<div class="tmpl-wp-desc">The '.get_current_theme().' theme is ideal for creating and monetizing an online listings directory to list hotels around the world. To help you setup the theme, please refer to its <a href="http://templatic.com/docs/splendor-theme-guide/">Installation Guide</a> to help you better understand the theme&#39;s functions. To help you get started, we have outlined a few recommended steps below to get you going. Should you need any assistance please also visit the Listings <a href="http://templatic.com/forums/viewforum.php?f=158">support forum</a> or use <a href="http://templatic.com/docs/submit-a-ticket/">helpdesk</a>. Need some extra features added to the theme? Please check out all of the available add-ons for it <a href="http://templatic.com/plugins/directory-add-ons/"> here</a>. </div>';
		
		/* guilde and support link */
		
		$dummy_data_msg .= apply_filters('tmpl_after_install_delete_button',$dummy_data_msg);	
		
		$dummy_nstallation_link  = '<div class="tmpl-ai-btm-links clearfix"><ul><li>Need Help?</li><li><a href="http://templatic.com/docs/splendor-theme-guide/">Theme & Installation Guide</a></li><li><a href="http://templatic.com/forums/viewforum.php?f=158">Support Forum</a></li><li><a href="http://templatic.com/docs/submit-a-ticket/">HelpDesk</a></li></ul><p><a href="http://templatic.com">Team Templatic</a> at your service</p></div>';
	...
	<hr>
		...
		/* if($post_counts>0) 	{*/
			...
			if($t_theme_licence_key_ ==''){
				...
				$licencekey_frm = '	<tr>
						...
						<td colspan="2"><h2><span>'.__('1', 'templatic').'</span>'.__('Licence Key Verification.', 'templatic').'</h2></td>
					...
					$licencekey_frm .= '<h2><span>'.__('2', 'templatic').'</span>' .__('Sample data', 'templatic').'</h2>
				...
				$licencekey_frm = '
					...
					<h2><span>'.__('1', 'templatic').'</span>'.__('Licence Key Verification.', 'templatic').'</h2>
		...
		if(isset($_REQUEST['dummy_insert']) && $_REQUEST['dummy_insert']){
			...
			/* get footer pages link */
			$about = $wpdb->get_row("SELECT * FROM $wpdb->posts where post_name like 'about' and post_type='page' and post_status='publish'");
			$contact = $wpdb->get_row("SELECT * FROM $wpdb->posts where post_name like 'contact' and post_type='page' and post_status='publish'");
			...
			if(!$nav_menus){
				...
				if(!in_array('primary',$navmenu)){
					...
					if($terms){
						...
						foreach($terms as $term){
							if($term->slug == 'places'){
								...
								$primary_post_info[] = array('post_title'=>$term->name,'post_content'=>$term->description,'post_id' =>$term->term_id,'_menu_item_type'=>'taxonomy','_menu_item_object'=>'listingcategory','menu_item_parent'=>$menu_item_parent);
							$i++;
							}
						}
					}
					...
					/*Insert primary menu */	
					tmpl_splendor_insert_menu_auto_install($primary_post_info,'primary');					
				...
				if(!in_array('footer',$navmenu)){
					...
					$footer_links[] = array('post_title' => 'Home', 'post_id' =>$front_page_id,'_menu_item_type'=>'post_type','_menu_item_object'=>'page','menu_item_parent'=>0);

					$footer_links[] = array('post_title' => $about->post_title, 'post_id' =>$about->ID,'_menu_item_type'=>'post_type','_menu_item_object'=>'page','menu_item_parent'=>0);
					$footer_links[] = array('post_title' => $contact->post_title, 'post_id' =>$contact->ID,'_menu_item_type'=>'post_type','_menu_item_object'=>'page','menu_item_parent'=>0);
					
										
					/*Insert footer menu */	
					tmpl_splendor_insert_menu_auto_install($footer_links,'footer');
				...
				if(!in_array('secondary',$navmenu)){
					...
					/* custom url */
					$secondary_post_info[] = array('post_title'=>'Category','post_content'=>'','post_id' =>'','_menu_item_type'=>'custom','_menu_item_object'=>'custom','menu_item_parent'=>0,'_menu_item_url'=>'#');
					/*Get the  listing category list */
					$args = array( 'taxonomy' =>'listingcategory','orderby'=> 'name','order' => 'ASC', );
					$terms = get_terms('listingcategory', $args);
					if($terms){
						$i=0;
						foreach($terms as $term){
					...
					/*Blog menu */
					$args1 = array( 'taxonomy' =>'category','orderby'=> 'id','order' => 'ASC','exclude'=>array('1'));
					$terms1 = get_terms('category', $args1);
					...
					if($terms1){
						...
						foreach($terms1 as $term1){
							$menu_item_parent1=1;
							$secondary_post_info[] = array('post_title'=>'','post_content'=>$term1->description,'post_id' =>$term1->term_id,'_menu_item_type'=>'taxonomy','_menu_item_object'=>'category','menu_item_parent'=>$menu_item_parent1);
							$i++;
						}
					}
					/*finish blog menu */
					...
					$contact_us_id = $wpdb->get_row("SELECT ID FROM $wpdb->posts WHERE $wpdb->posts.post_name = 'contact' and $wpdb->posts.post_type = 'page'");
					...
					tmpl_splendor_insert_menu_auto_install($secondary_post_info,'secondary');
				}
			}
		...
		if(isset($_REQUEST['dummy']) && $_REQUEST['dummy']=='del'){
			tmpl_splendor_delete_dummy_data();
...
*/
function tmpl_splendor_delete_dummy_data()
{
	global $wpdb;
	/* delete widgets entry from database */
...
add_action( 'wp_ajax_hide_admin_notification', 'tmpl_splendor_hide_admin_notification' );
add_action( 'wp_ajax_hide_authoinstall_notification', 'tmpl_splendor_hide_autoinstall_notification' );
function activate() {
	add_option( 'hide_ajax_notification', false );
...
*/
function tmpl_splendor_set_page_info_autorun($pages_array,$page_info_arr)
{
...
function tmpl_splendor_insert_menu_auto_install($post_info,$menu_type){	
...
add_filter('tevoluton_overviewbox_datacontent','tmpl_splendor_overviewbox_datacontent');
function tmpl_splendor_overviewbox_datacontent($dummy_data_msg){
	global $wpdb;	
	...
	if((strstr($_SERVER['REQUEST_URI'],'themes.php') && !isset($_REQUEST['page'])) && @$_REQUEST['template']==''){
		if($post_counts>0){
			$dummy_data_msg .= '<div id="install-notification" class="tmpl-auto-install-yb" ><h4>'.__("I don&rsquo;t need it, please",'templatic-admin').' </h4> <span><a class="button button-primary delete-data-button" href="'.home_url().'/wp-admin/themes.php?dummy=del">'.__('Delete sample data','templatic-admin').'</a></span> <p>Please note that deleting sample data will also remove any content you may have added or edited on any sample content. Deleting the sample data will also remove all sample widgets which were inserted.</p>';
			$dummy_data_msg .='<span id="install-notification-nonce" class="hidden">' . wp_create_nonce( 'install-notification-nonce' ) . '</span></div>';

		}else{
			$dummy_data_msg .= '<div id="install-notification" class="tmpl-auto-install-yb" ><h4>'.__("Your site not looking like our online demo?",'templatic-admin').' </h4> <span><a class="button button-primary" href="'.home_url().'/wp-admin/themes.php?dummy_insert=1">'.__('Install sample data','templatic-admin').'</a></span> <p>Please note that deleting sample data will also remove any content you may have added or edited on any sample content. Deleting the sample data will also remove all sample widgets which were inserted.</p>';
			$dummy_data_msg .='<span id="install-notification-nonce" class="hidden">' . wp_create_nonce( 'install-notification-nonce' ) . '</span></div>';
